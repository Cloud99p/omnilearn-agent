...
const testCases = [
  ...
  { input: "i dont understand, explain it more clearly", shouldExtract: false, reason: "clarification request" },
  { input: "I don't understand", shouldExtract: false, reason: "understanding statement" },
  ...
  { input: "I'm confused", shouldExtract: false, reason: "confusion statement" },
  { input: "hello", shouldExtract: false, reason: "greeting" },
  { input: "thanks", shouldExtract: false, reason: "gratitude" },
  { input: "let's talk about AI", shouldExtract: false, reason: "meta-conversation" },
  { input: "quick question", shouldExtract: false, reason: "meta-conversation" },
  
  // SHOULD EXTRACT (actual facts/statements)
  { input: "OmniLearn is an open-source AI agent", shouldExtract: true, reason: "declarative fact" },
  { input: "TypeScript is better than JavaScript", shouldExtract: true, reason: "opinion/fact" },
  { input: "I use GitHub with username Cloud99p", shouldExtract: true, reason: "user fact" },
  { input: "The database uses PostgreSQL", shouldExtract: true, reason: "technical fact" },
  { input: "OmniLearn was created by Emmanuel", shouldExtract: true, reason: "creator fact" },
];
...
for (const { input, shouldExtract, reason } of testCases) {
  ...
  if (success) {
    ...
    console.log(`❌ "${input}"`);
    console.log(`   → Expected: ${shouldExtract ? 'extract' : 'skip'}, got: ${hasFacts ? 'extract' : 'skip'}`);
    console.log(`   → Reason: ${reason}`);
    if (facts.length > 0) {
      console.log(`   → Extracted: ${facts.map(f => f.content.slice(0, 50)).join(", ")}...`);
    }
    failed++;
  }
  console.log();
