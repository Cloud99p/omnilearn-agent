...
async function main() {
  ...
  // Fetch all nodes
  const allNodes = await db.select().from(knowledgeNodes).orderBy(desc(knowledgeNodes.createdAt));
  
  console.log(`📊 Found ${allNodes.length} total nodes`);
  
  // Categorize by embedding status
  const withEmbeddings = allNodes.filter(n => n.embedding && Array.isArray(n.embedding) && n.embedding.length > 0);
  const withoutEmbeddings = allNodes.filter(n => !n.embedding || !Array.isArray(n.embedding) || n.embedding.length === 0);
  ...
  const backup = {
    ...
    nodes: allNodes.map(n => ({
      ...
      tokens: n.tokens,
      hasEmbedding: !!(n.embedding && Array.isArray(n.embedding) && n.embedding.length > 0),
  ...
  // Write backup file
  const backupPath = join(process.cwd(), `knowledge-nodes-backup-${Date.now()}.json`);
  writeFileSync(backupPath, JSON.stringify(backup, null, 2));
  
  console.log(`✅ Backup saved to: ${backupPath}`);
  console.log(`📁 File size: ${(Buffer.byteLength(JSON.stringify(backup), 'utf8') / 1024).toFixed(2)} KB`);
  ...
  const reingestPath = join(process.cwd(), `nodes-to-reingest-${Date.now()}.json`);
