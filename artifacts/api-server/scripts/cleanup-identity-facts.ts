...
async function cleanup() {
  ...
  confusedNodes.forEach((node, i) => {
    console.log(
      `${i + 1}. [${node.type}] ${node.content.slice(0, 100)}${node.content.length > 100 ? "..." : ""}`,
    );
    console.log(
      `   ID: ${node.id} | clerkId: ${node.clerkId || "NULL"} | similarity: ${(node as any).similarity || "N/A"}`,
    );
    console.log();
  });
  ...
  if (idsToDelete.length > 0) {
    await db
      ...
      .where(or(...idsToDelete.map((id) => eq(knowledgeNodes.id, id))));
  ...
  if (remainingOrphans.length > 0 && remainingOrphans.some((n) => !n.clerkId)) {
    ...
    console.log(
      "   These should be reviewed manually or assigned to appropriate users",
    );
  }
