...
if (!BACKUP_FILE) {
  console.error("❌ Usage: pnpm tsx scripts/reingest-knowledge-nodes.ts <backup-file.json>");
  console.error("   Example: pnpm tsx scripts/reingest-knowledge-nodes.ts knowledge-nodes-backup-1234567890.json");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`📊 Nodes already with embeddings: ${nodesWithEmbeddings.length} (skipping)`);
  ...
  for (let i = 0; i < nodesToReingest.length; i += BATCH_SIZE) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      try {
        ...
        if (inserted % 20 === 0) {
          console.log(`   ✓ Inserted ${inserted}/${nodesToReingest.length} nodes...`);
        }
      } catch (err) {
