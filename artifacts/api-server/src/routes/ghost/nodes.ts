...
router.get("/status", async (req, res) => {
  try {
    ...
    const avgMs =
      nodes.filter((n) => n.avgResponseMs).length > 0
        ? nodes
            .filter((n) => n.avgResponseMs)
            .reduce((s, n) => s + n.avgResponseMs!, 0) /
...
router.post("/nodes", async (req, res) => {
  ...
  if (!name?.trim() || !endpoint?.trim() || !secretKey?.trim()) {
    res
      .status(400)
      .json({ error: "name, endpoint, and secretKey are required" });
    return;
  }
