...
router.post("/chat", async (req, res) => {
  ...
  try {
    ...
    const availableBrowserWorkers = allWorkers.filter(
      (w) => w.lastSeen && now - new Date(w.lastSeen).getTime() < 90_000,
    );
    ...
    if (availableServerNodes.length > 0) {
      ...
      if (!succeeded) {
        if (hasBrowserWorkers) {
          fullResponse = await routeToBrowserWorker(
            req,
            res,
            content.trim(),
            history,
          ...
          sendEvent(res, {
            fallback: true,
            reason: "All server nodes unreachable — running locally.",
          });
          fullResponse = await runLocalNative(history, content.trim(), res);
          sendEvent(res, {
            done: true,
            conversationId: convId,
            meta: { routed: false, local: true },
          });
        }
      }
...
async function routeToBrowserWorker(
  ...
  sendEvent(res, {
    fallback: true,
    reason: "Browser worker timed out — running locally.",
  });
