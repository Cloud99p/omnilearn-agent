    ...
    switch (method) {
      ...
      case "doc": {
        ...
        try {
          ...
          return {
            text: stdout,
            metadata: { method: "pandoc", originalName, type: "word" },
          };
        } catch {
          // Fallback: try to read as zip and extract text
          const text = await fs.readFile(filePath, "utf-8");
          return {
            text: `[Binary DOC/DOCX file: ${originalName}]`,
            metadata: { method: "binary", originalName },
          };
        }
      }
      ...
      case "xls": {
        ...
        try {
          ...
          return {
            text: stdout,
            metadata: { method: "ssconvert", originalName, type: "excel" },
          };
        } catch {
          return {
            text: `[Excel file: ${originalName} - install gnumeric for extraction]`,
            metadata: { method: "binary", originalName },
          };
        }
      }
      ...
      case "pptx": {
        try {
          ...
          return {
            text: stdout,
            metadata: { method: "pandoc", originalName, type: "powerpoint" },
          };
        } catch {
          return {
            text: `[PowerPoint file: ${originalName}]`,
            metadata: { method: "binary", originalName },
          };
        }
      }
      ...
      case "ocr": {
        ...
        try {
          ...
          return {
            text: stdout,
            metadata: { method: "tesseract", originalName, type: "image" },
          };
        } catch (err) {
          return {
            text: `[Image file: ${originalName} - install tesseract for OCR]`,
            metadata: { method: "binary", originalName },
          };
        }
      }
...
function chunkText(
  ...
  for (const para of paragraphs.slice(0, 50)) {
    ...
    let type = "fact";
    if (
      cleanPara.includes("how to") ||
      cleanPara.includes("steps") ||
      cleanPara.includes("procedure")
    ) {
      type = "procedure";
    } else if (
      cleanPara.includes("definition") ||
      cleanPara.includes("means") ||
      cleanPara.includes("refers to")
    ) {
      type = "concept";
    } else if (
      cleanPara.includes("opinion") ||
      cleanPara.includes("I think") ||
      cleanPara.includes("believe")
    ) {
      type = "opinion";
    }
...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    logger.info(
      { file: file.filename, originalName, mimeType, mode },
