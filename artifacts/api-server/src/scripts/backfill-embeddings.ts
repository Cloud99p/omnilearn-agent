...
async function backfillEmbeddings() {
  ...
  const nodesWithoutEmbedding = allNodes.filter(
    (n) => !n.embedding || n.embedding.length === 0,
  );
  logger.info(
    { total: allNodes.length, withoutEmbedding: nodesWithoutEmbedding.length },
    "Node counts",
  );
  ...
  for (let i = 0; i < nodesWithoutEmbedding.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        if (!embedding) {
          errors.push({ id: node.id, error: "No embedding generated" });
          continue;
        }
        await db
    ...
    if (i + BATCH_SIZE < nodesWithoutEmbedding.length)
      await new Promise((resolve) => setTimeout(resolve, DELAY_MS));
  }
  logger.info(
    { processed, errors: errors.length, total: allNodes.length },
    "Backfill complete",
  );
}
