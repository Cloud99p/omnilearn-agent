...
async function createOntologyProposal(opts: {
  ...
  const proposalProof = buildOntologyProof(
    opType,
    targetNodeId ?? null,
    targetNodeBId ?? null,
    rationaleHash,
...
export async function validateOntologyProposal(proposalId: number): Promise<{
  ...
  const recomputedProof = buildOntologyProof(
    p.opType,
    p.targetNodeId,
    p.targetNodeBId,
    p.rationaleHash,
  ...
  if (!valid) {
    const reason = !freshnessOk
      ? "Proposal older than 7 days"
      : !rationaleHashMatch
        ? "Rationale hash mismatch"
        : "Proof mismatch";
    return {
      valid,
      steps: { rationaleHashMatch, proofMatch, freshnessOk },
      reason,
    };
  }
...
export async function executeOntologyProposal(proposalId: number): Promise<{
  ...
  if (p.status !== "approved")
    return { ok: false, summary: "Proposal must be approved before execution" };
  ...
  try {
    ...
    if (p.opType === "new-edge-type" && p.proposedEdgeType) {
      ...
      const [nodeB] = await db
        .select()
        .from(knowledgeNodes)
        .where(eq(knowledgeNodes.id, p.targetNodeBId));
      if (!nodeA || !nodeB) {
        return { ok: false, summary: "One or both merge targets not found" };
      }

      // Create merged node
      const mergedContent = `${nodeA.content} [merged: ${nodeB.content}]`.slice(
        0,
        500,
      );
      ...
      await db
        ...
        .set({ fromId: merged.id })
        .where(eq(knowledgeEdges.fromId, nodeA.id));
      await db
        .update(knowledgeEdges)
        .set({ toId: merged.id })
        .where(eq(knowledgeEdges.toId, nodeA.id));
      await db
        .update(knowledgeEdges)
        .set({ fromId: merged.id })
        .where(eq(knowledgeEdges.fromId, nodeB.id));
      await db
        .update(knowledgeEdges)
        .set({ toId: merged.id })
        .where(eq(knowledgeEdges.toId, nodeB.id));
      ...
      if (parts.length < 2)
        parts = [
          target.content + " (aspect A)",
          target.content + " (aspect B)",
        ];
      ...
      if (newNodes[0]) {
        await db
          .update(knowledgeEdges)
          .set({ fromId: newNodes[0].id })
          .where(eq(knowledgeEdges.fromId, target.id));
        await db
          .update(knowledgeEdges)
          .set({ toId: newNodes[0].id })
          .where(eq(knowledgeEdges.toId, target.id));
      }
      ...
      return {
        ok: false,
        summary: `Unknown opType or missing fields: ${p.opType}`,
      };
    }

    await db
      .update(ontologyProposals)
      .set({ status: "executed", executedAt: new Date() })
      .where(eq(ontologyProposals.id, proposalId));

    logger.info(
      { proposalId, opType: p.opType, summary },
      "Ontology proposal executed",
    );
...
export async function runOntologyReflection(): Promise<{
  ...
  const existingVocab = await db
    .select()
    .from(ontologyNodes)
    .where(eq(ontologyNodes.nodeType, "edge-vocab"));
  const registeredEdgeTypes = new Set(existingVocab.map((n) => n.name));
  const coreEdgeTypes = [
    {
      name: "co-occurs",
      description: "Two concepts frequently appear together in observations.",
    },
    {
      name: "related-to",
      description: "Semantic similarity relationship between two concepts.",
    },
    {
      name: "causes",
      description: "One concept is a cause or antecedent of another.",
    },
    {
      name: "contradicts",
      description: "Two concepts are mutually exclusive or in tension.",
    },
    {
      name: "supports",
      description: "One concept provides evidence or backing for another.",
    },
  ];
  ...
  const edges = await db
    .select({ relationship: knowledgeEdges.relationship })
    .from(knowledgeEdges);
  ...
  for (const rel of usedRelationships) {
    if (!registeredEdgeTypes.has(rel)) {
      const alreadyProposed = await db
        ...
        .where(
          and(
            eq(ontologyProposals.opType, "new-edge-type"),
            eq(ontologyProposals.proposedEdgeType, rel),
          ),
        );
  ...
  const lowConfidenceRules = await db
    .select()
    .from(knowledgeNodes)
    .where(
      and(eq(knowledgeNodes.type, "rule"), lt(knowledgeNodes.confidence, 0.4)),
    )
    .limit(20);
  ...
  for (const node of lowConfidenceRules) {
    const alreadyProposed = await db
      ...
      .where(
        and(
          eq(ontologyProposals.opType, "demote-rule"),
          eq(ontologyProposals.targetNodeId, node.id),
  ...
  const conceptNodes = await db
    .select()
    .from(knowledgeNodes)
    .where(eq(knowledgeNodes.type, "concept"))
    .limit(200);
  ...
  for (const [nodeId, relSet] of nodeEdgeCounts) {
    ...
    const alreadySplitProposed = await db
      .select()
      .from(ontologyProposals)
      .where(
        and(
          eq(ontologyProposals.opType, "split-node"),
          eq(ontologyProposals.targetNodeId, nodeId),
          ne(ontologyProposals.status, "rejected"),
        ),
      );
