...
import { Globe, Database, BrainCircuit, Activity, Settings, Code, Zap, Shield, ChevronDown } from "lucide-react";
...
const MODULES = [
  {
    ...
    name: "Data Ingestion",
    icon: Globe,
    purpose: "Continuously crawls, scrapes, and streams public internet data — web pages, APIs, RSS feeds, scientific repositories, social media, government data, and more. Each source type is handled by a dedicated, swappable connector.",
    stack: [
    ...
    scaling: "Horizontal: deploy additional Scrapy spiders as containers. Kafka partitioning distributes load across workers. Connector registry allows hot-swapping without downtime.",
...
async def ingestion_loop(source_config):
  ...
  },
  {
    ...
    icon: Database,
    purpose: "The persistent semantic memory of OmniLearn. Stores vectorized embeddings of all ingested content enabling semantic search, similarity queries, and structured knowledge retrieval at scale.",
    stack: [
      { name: "Chroma", role: "Embedded vector database for smaller deployments" },
    ...
    scaling: "Vector stores support horizontal sharding. Hot/warm/cold tiering: recent embeddings in fast memory, older in compressed disk. Prioritization logic decides what stays hot.",
...
def store_with_priority(document, embedder):
  ...
  },
  {
    ...
    icon: BrainCircuit,
    purpose: "Transforms raw indexed data into structured knowledge. Runs three distinct modes: passive indexing (store everything), active extraction (pull entities, relationships, facts), and deep synthesis (cross-source reasoning and insight generation).",
    stack: [
    ...
    scaling: "Fine-tuning runs in isolated jobs on GPU nodes. RAG retrieval is stateless and horizontally scalable. Catastrophic forgetting mitigated via LoRA adapters — base model unchanged.",
...
def update_character(learning_event, character_state):
  ...
  },
  {
    ...
    icon: Settings,
    purpose: "The control plane of OmniLearn. Watches YAML/JSON config files for changes, validates against schema, and applies updates live without restarting the core loop. All changes are logged and diff-tracked.",
    stack: [
    ...
    scaling: "Stateful singleton per deployment. In a cluster, config is stored in a shared volume or etcd and changes broadcast via Kafka events to all nodes.",
...
class ConfigManager:
  ...
  },
  {
    ...
    icon: Code,
    purpose: "Exposes OmniLearn's capabilities to the outside world. REST and gRPC endpoints for querying knowledge, triggering ingestion, managing personas, and monitoring system state. Stateless and horizontally scalable.",
    stack: [
    ...
    scaling: "Stateless: spin up as many instances as needed behind a load balancer. Rate limiting enforced at this layer. Long-running jobs dispatched to Kafka queues.",
...
async def query(request: QueryRequest):
  ...
  },
  {
    ...
    icon: Zap,
    purpose: "OmniLearn's self-improvement loop. Monitors system performance, detects knowledge gaps, and autonomously triggers retraining or reconfiguration. All self-modifications are sandboxed before promotion.",
    stack: [
    ...
    scaling: "Experiment sandboxes run as isolated containers. Promotion to production requires passing a benchmark suite. All experiments logged in MLflow with full reproducibility.",
...
async def meta_loop():
  ...
  },
  {
    ...
    icon: Shield,
    purpose: "Enforces ethical and legal boundaries: robots.txt respect, noindex tags, rate limits, terms of service constraints. An ethics governor configurable from 'strict politeness' to 'unfiltered researcher' — neither mode enables illegal activity.",
    stack: [
      ...
      { name: "urllib.robotparser", role: "Standard library robots.txt support" },
      { name: "structlog", role: "Compliance audit trail" },
    ],
    scaling: "Stateless filter layer. Deployed as middleware in the ingestion pipeline. Rate limit state stored in Redis for distributed coordination across crawler nodes.",
  ...
  return (
    <div className="p-6 md:p-12 max-w-5xl mx-auto min-h-screen">
      <motion.div
        ...
        className="mb-12"
      >
        <h1 className="text-4xl font-bold tracking-tight mb-4">Component Breakdown</h1>
        ...
        {MODULES.map((mod, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className={`p-2 rounded-md ${isOpen ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"}`}>
                  <mod.icon className="w-5 h-5" />
                </div>
                <span className="font-mono font-bold text-lg flex-1">{mod.name}</span>
                ...
                {isOpen && (
                  ...
                  >
                    <div className="px-5 pb-6 space-y-6 border-t border-border/50 pt-5">
                      <p className="text-muted-foreground leading-relaxed">{mod.purpose}</p>

                      <div className="grid md:grid-cols-2 gap-6">
                        <div>
                          <h4 className="font-mono text-sm text-primary uppercase tracking-wider mb-3">Tech Stack</h4>
                          <div className="space-y-2">
                            {mod.stack.map(s => (
                              <div key={s.name} className="flex items-start gap-2">
                                <span className="font-mono text-sm text-primary mt-0.5 shrink-0">{s.name}</span>
                        ...
                        <div>
                          <h4 className="font-mono text-sm text-primary uppercase tracking-wider mb-3">Scaling Strategy</h4>
                          <p className="text-muted-foreground text-sm leading-relaxed">{mod.scaling}</p>
                        </div>
                      </div>

                      <div>
                        <h4 className="font-mono text-sm text-primary uppercase tracking-wider mb-3">Core Loop</h4>
                      ...
                      <div>
                        <h4 className="font-mono text-sm text-primary uppercase tracking-wider mb-3">Resource Estimates</h4>
                        <div className="grid grid-cols-3 gap-3 mb-4">
                          {mod.resources.map(r => (
                            <div key={r.label} className="bg-background border border-border rounded-md p-3 text-center">
                              <div className="font-mono text-xs text-muted-foreground mb-2">{r.label}</div>
                              <div className="text-sm font-bold text-primary">{r.cpu} CPU</div>
                              <div className="text-xs text-muted-foreground">{r.ram} GB RAM</div>
                              <div className="text-xs text-muted-foreground">{r.storage} GB storage</div>
                            </div>
                          ))}
                        </div>
                        <div className="h-32">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={mod.resources} barSize={20}>
                              <XAxis dataKey="label" tick={{ fontSize: 10, fill: "hsl(215 20.2% 65.1%)", fontFamily: "monospace" }} axisLine={false} tickLine={false} />
                              <YAxis tick={{ fontSize: 10, fill: "hsl(215 20.2% 65.1%)", fontFamily: "monospace" }} axisLine={false} tickLine={false} />
                              <Tooltip contentStyle={TOOLTIP_STYLE} />
                              <Bar dataKey="ram" name="RAM (GB)" fill="hsl(180 100% 45%)" radius={[2, 2, 0, 0]} />
                              <Bar dataKey="cpu" name="CPU cores" fill="hsl(260 100% 65%)" radius={[2, 2, 0, 0]} />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
