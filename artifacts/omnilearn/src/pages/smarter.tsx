...
export default function SmarterPage() {
  ...
  const passes = data
    ? [
        data.pareto.verdict,
        data.confidence.verdict,
        data.precision.verdict,
        data.graph.verdict,
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-muted-foreground font-mono text-sm max-w-2xl leading-relaxed">
              Growing a knowledge graph is easy. Proving it actually improves
              reasoning is harder. Four statistical tests — Pareto
              concentration, confidence gradient, retrieval precision lift, and
              graph density — each independent evidence that intelligence is
              compounding.
            </p>
          </div>
          <button
      ...
      {!loading && data && (data as { empty?: boolean }).empty && (
        <div className="bg-secondary/20 border border-border rounded-xl px-6 py-10 text-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Chat with the agent or train it via Intelligence → the proofs will
            run automatically once nodes exist.
          </p>
        </div>
      )}
          ...
          >
            ...
            <div className="flex items-center gap-6 bg-card border border-border rounded-xl px-5 py-4 flex-wrap">
              {[
                {
                  l: "Knowledge nodes",
                  v: data.snapshot.totalNodes,
                  icon: Brain,
                },
                {
                  l: "Graph edges",
                  v: data.snapshot.totalEdges,
                  icon: GitBranch,
                },
                {
                  l: "Total accesses",
                  v: data.snapshot.totalAccesses,
                  icon: Activity,
                },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <p className="text-xs text-muted-foreground leading-relaxed">
                  If knowledge were accumulating randomly, accesses would spread
                  evenly — each 20% of nodes handles 20% of queries. When a core
                  set of high-value nodes handles a much larger share, the agent
                  has learned{" "}
                ...
                <div className="space-y-2 pt-1">
                  ...
                  <MiniBar
                    value={
                      data.pareto.totalAccesses - data.pareto.top20Accesses
                    }
                    max={data.pareto.totalAccesses || 1}
                    color="#334155"
              ...
              >
                <p className="text-xs text-muted-foreground leading-relaxed">
                  The most-used nodes should also be the most reliable ones. A
                  positive gradient proves the retrieval system is surfacing{" "}
                  <em className="text-foreground">high-confidence facts</em>,
                  not grabbing random noise.
                </p>
                <div className="flex items-end gap-1 h-12 pt-2">
                  {data.confidence.histogram.map((b, i) => {
                    const maxCount = Math.max(
                      ...data.confidence.histogram.map((x) => x.count),
                      1,
                    );
                    ...
                    return (
                      ...
                      >
                        <motion.div
                          ...
                          style={{
                            backgroundColor: `hsl(${180 + i * 15}, 70%, ${35 + i * 8}%)`,
                          }}
                          initial={{ height: 0 }}
                          animate={{ height: `${pct}%` }}
              ...
              >
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Five domain-specific questions are probed against the
                  knowledge graph. A random retriever would score ≈{" "}
                  {(data.precision.randomBaseline * 100).toFixed(0)}% relevance.
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <p className="font-mono text-[9px] text-muted-foreground/50 pt-1">
                    {data.precision.probes
                      .map((p) => p.question.slice(0, 30) + "…")
                      .join(" · ")}
                  </p>
                </div>
              </ProofCard>
              ...
              >
                <p className="text-xs text-muted-foreground leading-relaxed">
                  A growing graph that just accumulates facts stays sparse. A
                  graph that's building{" "}
                  <em className="text-foreground">conceptual connections</em>{" "}
                  grows denser edges between nodes — evidence of structured
                  understanding, not trivia hoarding.
                </p>
              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 px-5 py-4 border-b border-border/50">
                  ...
                  <span className="font-mono text-[10px] text-muted-foreground ml-2">
                    — the most-retrieved facts. High access = the agent keeps
                    returning here because these nodes answer real questions.
                  </span>
                </div>
