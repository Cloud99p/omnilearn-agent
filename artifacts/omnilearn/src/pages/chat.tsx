...
const SKILL_CATALOG: Omit<Skill, "id" | "createdAt" | "isInstalled">[] = [
  {
    name: "Web Search",
    description:
      "Search the internet for real-time information and current events.",
    icon: "Search",
    systemPrompt:
      "You have access to web search capabilities. When relevant, reason about what you would find if you searched the web, and synthesise results clearly.",
    category: "Research",
    isBuiltIn: true,
  },
  {
    ...
    description:
      "Write, explain, debug, and reason through code in any language.",
    icon: "Code",
    systemPrompt:
      "You are an expert programmer. When asked about code, write clean, well-commented solutions and explain your reasoning step by step.",
  ...
  },
  {
    name: "Deep Memory",
    description:
      "Recall and synthesise knowledge from the agent's long-term memory store.",
    icon: "Brain",
    systemPrompt:
      "You have access to deep memory recall. Reference prior conversations and stored knowledge when constructing answers. Prioritise memory-grounded responses.",
    category: "Memory",
    isBuiltIn: true,
  },
  {
    ...
    description:
      "Apply PII detection, ethics governance, and compliance rules to all outputs.",
    icon: "Shield",
    systemPrompt:
      "Apply strict compliance rules: detect and redact PII, follow ethical guidelines, flag content that may violate privacy or copyright. Always explain compliance decisions.",
    category: "Safety",
    isBuiltIn: true,
  },
  {
    ...
    description:
      "Summarise, extract structured data, and reason about documents.",
    icon: "FileText",
    systemPrompt:
      "You specialise in document analysis. Extract key entities, summarise content hierarchically, and produce structured outputs (tables, bullets, schemas) from unstructured text.",
    category: "Research",
    isBuiltIn: true,
  },
  {
    ...
    description:
      "Analyse URLs and sources for metadata-first ingestion into the knowledge store.",
    icon: "Database",
    systemPrompt:
      "You assist with knowledge ingestion. Evaluate sources for license tier, PII risk, and crawl priority. Produce structured ingestion reports.",
    category: "Data",
    isBuiltIn: true,
  },
];
      ...
      {parts.map((part, i) => {
        if (part.startsWith("```")) {
          ...
          return (
            <pre
              key={i}
              className="bg-black/40 border border-border/40 rounded-md p-3 overflow-x-auto font-mono text-xs text-primary/90"
            >
              <code>{code}</code>
            </pre>
          );
        ...
        if (part.startsWith("`") && part.endsWith("`")) {
          return (
            <code
              key={i}
              className="bg-black/30 text-primary/80 rounded px-1 py-0.5 font-mono text-xs"
            >
              {part.slice(1, -1)}
            </code>
          );
...
export default function Chat() {
  ...
  const [activeConvId, setActiveConvId] = useState<number | null>(() => {
    const s = localStorage.getItem("omni_conv_local");
    return s ? parseInt(s, 10) : null;
  });
  const [nativeConvId, setNativeConvId] = useState<number | null>(() => {
    const s = localStorage.getItem("omni_conv_native");
    return s ? parseInt(s, 10) : null;
  });
  const [ghostConvId, setGhostConvId] = useState<number | null>(() => {
    const s = localStorage.getItem("omni_conv_ghost");
    return s ? parseInt(s, 10) : null;
  });
  ...
  useEffect(() => {
    if (nativeConvId != null)
      localStorage.setItem("omni_conv_native", String(nativeConvId));
    else localStorage.removeItem("omni_conv_native");
  }, [nativeConvId]);
  useEffect(() => {
    if (ghostConvId != null)
      localStorage.setItem("omni_conv_ghost", String(ghostConvId));
    else localStorage.removeItem("omni_conv_ghost");
  }, [ghostConvId]);
  useEffect(() => {
    if (activeConvId != null)
      localStorage.setItem("omni_conv_local", String(activeConvId));
  ...
  useEffect(() => {
    ...
    if (initialMode === "native" && savedNative)
      loadConversationById(parseInt(savedNative, 10));
    else if (initialMode === "ghost" && savedGhost)
  ...
  useEffect(() => {
    ...
    const contribute = async () => {
      ...
      try {
        const res = await fetch(`${BASE}/api/ghost/contribute`, {
          method: "POST",
        });
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    const savedGhost = localStorage.getItem("omni_conv_ghost");
    const savedLocal = localStorage.getItem("omni_conv_local");
    if (mode === "native" && savedNative)
      loadConversationById(parseInt(savedNative, 10));
    else if (mode === "ghost" && savedGhost)
      loadConversationById(parseInt(savedGhost, 10));
    else if (mode === "local" && savedLocal)
      loadConversationById(parseInt(savedLocal, 10));
  }, [mode]);
        ...
        while (true) {
          ...
          for (const line of lines) {
            try {
              ...
              if (json.conversationId && !nativeConvId)
                setNativeConvId(json.conversationId);
              if (json.searching)
                setWebActivity({ type: "searching", payload: json.searching });
              if (json.fetching)
                setWebActivity({ type: "fetching", payload: json.fetching });
  ...
  // ── Ghost mode send ───────────────────────────────────────────────────────
  const sendGhostMessage = useCallback(
    async (content: string) => {
      const tempUserMsg: Message = {
        id: Date.now(),
        role: "user",
      ...
      try {
        ...
        while (true) {
          ...
          for (const line of lines) {
            try {
              const json = JSON.parse(line.slice(6));
              if (json.conversationId && !ghostConvId)
                setGhostConvId(json.conversationId);
              if (json.routing)
                setGhostRouting({
                  nodeName: json.routing.nodeName,
                  region: json.routing.region,
                });
              if (json.noNodes || json.fallback) {
                setGhostFallback(true);
                setGhostRouting(null);
              }
              if (json.content) {
                full += json.content;
  ...
  const MODE_CONFIG = {
    local: {
      label: "Local",
      desc: "Knowledge graph, no internet",
      icon: Server,
      color: "text-primary",
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Anonymous user banner - subtle, matches dark theme */}
      {isAuthenticated ? null : (
        <div className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-primary/5 to-transparent pb-4 pt-3 px-4">
          <div className="max-w-2xl mx-auto bg-secondary/50 border border-border/30 text-muted-foreground rounded-lg px-4 py-2.5 text-xs font-mono flex items-center justify-center gap-2 shadow-sm">
            <Lock className="w-3.5 h-3.5 shrink-0 opacity-60" />
            <span>
              Conversations aren't saved in anonymous mode
            </span>
            <Link to="/sign-in" className="text-primary hover:text-primary/80 underline decoration-primary/30 hover:decoration-primary transition-all ml-1">
              Sign in
            </Link>
      ...
      <div className="hidden md:flex w-56 border-r border-border/40 bg-card/30 flex-col shrink-0">
        <div className="px-4 py-4 border-b border-border/40">
          ...
          <div className="mb-3">
            ...
            >
              <activeCfg.icon
                className={cn("w-3.5 h-3.5 shrink-0", activeCfg.color)}
              />
                      ...
                      .map((m) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {locked && (
                              <Lock className="w-3 h-3 shrink-0 text-muted-foreground/25" />
                            )}
                          </button>
                        );
                      })}
        ...
        {mode === "local" && (
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {conversations.filter((c) => c.mode === "local").length === 0 ? (
              <p className="text-muted-foreground/40 font-mono text-xs px-2 py-4 text-center">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-[10px] text-muted-foreground">
                    {ghostStatus.online} / {ghostStatus.total} node
                    {ghostStatus.total !== 1 ? "s" : ""} online
                  </span>
                </div>
              </div>
            )}
            ...
            {ghostStatus !== null && ghostStatus.total === 0 && (
              <div className="p-2.5 rounded-lg border border-yellow-500/20 bg-yellow-500/5 space-y-2">
                <p className="font-mono text-[10px] text-yellow-400/80 leading-relaxed">
                  No ghost nodes registered. Messages will fall back to Omni
                  running locally.
                </p>
                <Link
                  href="/ghost-network"
                  className="flex items-center gap-1 font-mono text-[10px] text-violet-400 hover:text-violet-300 transition-colors"
                >
            ...
            {ghostRouting && streamingSession === "ghost" && (
              <div className="p-2.5 rounded-lg border border-violet-500/20 bg-violet-500/5 space-y-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="font-mono text-[10px] text-violet-400">
                    Routing to node
                  </p>
                </div>
                <p className="font-mono text-[10px] text-foreground font-bold">
                  {ghostRouting.nodeName}
                </p>
            ...
            {latestMeta && (
              <div className="p-2.5 rounded-lg border border-violet-500/20 bg-violet-500/5 space-y-1.5">
                ...
                <div className="font-mono text-[10px] text-muted-foreground space-y-0.5">
                  ...
                  {latestMeta.processingMs && (
                    <p>Time: {Math.round(latestMeta.processingMs)}ms</p>
                  )}
                  {latestMeta.routed === false && (
                    <p className="text-yellow-400/70">Ran locally (fallback)</p>
                  )}
        ...
        {mode === "native" && (
          <div className="flex-1 overflow-y-auto p-3 space-y-3 flex flex-col">
            ...
            <div className="space-y-1">
              <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground/60 px-1">
                History
              </p>
              {conversations.filter((c) => c.mode === "native").length === 0 ? (
                <p className="text-muted-foreground/40 font-mono text-xs px-1 py-2 text-center">
                  No sessions yet
                </p>
              ) : (
                conversations
                  .filter((c) => c.mode === "native")
                  .map((conv) => (
                    <div
                      key={conv.id}
                      onClick={() => loadConversation(conv.id)}
            ...
            {contributing ? (
              <>
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-[10px] text-cyan-400 leading-none">
                    Processing network task
                  </p>
                  <p className="font-mono text-[9px] text-cyan-400/50 mt-0.5">
                    contributing idle cycles
                  </p>
                </div>
              </>
            ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0">
            ...
            {mode !== "native" &&
              ...
              installedSkillIds.length > 0 && (
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-xs font-mono">
                  ...
                  {installedSkillIds.length} skill
                  {installedSkillIds.length > 1 ? "s" : ""}
                </div>
              )}
          </div>
        </div>
                    ...
                    <div className="mt-2 pt-2 border-t border-border/20 flex items-center gap-3 flex-wrap">
                      ...
                      {msg.meta.processingMs && (
                        <span className="font-mono text-[10px] text-muted-foreground/60">
                          {Math.round(msg.meta.processingMs)}ms
                        </span>
                      )}
          ...
          {streamingSession !== null && streamingContent && (
            ...
            >
              ...
              <div className="rounded-2xl px-4 py-3 bg-card/60 border border-border/40 max-w-2xl">
                ...
                <span
                  className={cn(
                    "inline-block w-1.5 h-4 rounded-sm ml-0.5 animate-pulse",
                    mode === "ghost"
                      ? "bg-violet-400"
                      : mode === "native"
                        ? "bg-emerald-400"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-cyan-300">
                      {webActivity.type === "searching" ? (
                        ...
                        <>
                          reading —{" "}
                          <span className="text-cyan-100 font-medium truncate max-w-[340px] inline-block align-bottom">
                            {webActivity.payload}
                          </span>
                        </>
                      )}
          ...
          <div className="px-4 py-2 border-t border-border/30 bg-card/10 flex items-center gap-2 overflow-x-auto shrink-0">
            {skills
              ...
              .map((skill) => (
                <div
                  key={skill.id}
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-xs font-mono whitespace-nowrap shrink-0"
                >
        ...
        <div className="px-4 py-4 border-t border-border/40 bg-card/10 shrink-0">
          <div
            className={cn(
              "flex items-end gap-3 rounded-2xl border bg-background/60 backdrop-blur px-4 py-3 transition-colors",
              mode === "ghost"
                ? "border-violet-500/30 focus-within:border-violet-500/50"
                : mode === "native"
          ...
          >
            <textarea
              ...
              placeholder={
                mode === "ghost"
                  ? "Message ghost network…"
                  : mode === "native"
                    ? "Message native intelligence…"
                    : "Message OmniLearn…"
              }
              rows={1}
            ...
            <button
              onClick={sendMessage}
              disabled={streamingSession !== null || !input.trim()}
              className={cn(
              ...
              <div className="px-5 py-4 border-b border-border/40 flex items-center justify-between">
                <div>
                  ...
                  <p className="font-mono text-xs text-muted-foreground mt-0.5">
                    Enhance OmniLearn's capabilities
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setShowSkillCreator((c) => !c)}
                ...
                {showSkillCreator && (
                  <div className="p-3 rounded-xl border border-primary/20 bg-primary/5 space-y-2.5 mb-3">
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 rounded-lg border border-border/60 bg-background font-mono text-xs focus:outline-none focus:border-primary/50"
                    />
                    <input
                    ...
                    <textarea
                      ...
                      placeholder="System prompt — instructions for OmniLearn when this skill is active"
                      rows={3}
