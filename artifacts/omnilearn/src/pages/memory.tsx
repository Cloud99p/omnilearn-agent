...
const DEPTH_SPEC: Record<StorageDepth, DepthSpec> = {
  full_text: {
    ...
    description:
      "Full extracted text, chunked embeddings, and all metadata. Served from NVMe. Reserved for documents that are actively referenced and deeply trusted.",
    costPerDoc: "~52 KB",
    stored: [
      "Canonical URL",
      "Full extracted text",
  ...
  metadata: {
    ...
    description:
      "Summary, embedding, and provenance. Fast retrieval without full text. Most of the recall value at a fraction of the storage cost.",
    costPerDoc: "~1.8 KB",
    stored: [
      "Canonical URL",
      "Model summary (3–5 sentences)",
      "Document-level embedding (1536-dim)",
      "Title + OG metadata",
      "Trust score + provenance",
    ],
    dropped: [
  ...
  summary_only: {
    ...
    description:
      "A single-sentence distillation and the URL. The document still answers broad recall queries. Re-crawl on demand restores full metadata within seconds.",
    costPerDoc: "~0.4 KB",
    stored: [
      "Canonical URL",
      "One-line distillation",
      "Document-level embedding (quantised, 384-dim)",
      "Trust score",
    ],
    dropped: [
      "Full extracted text",
  ...
  url_only: {
    ...
    description:
      "Just the URL and its crawl timestamp. The agent has not forgotten this document — it knows exactly where it lives. Re-crawl on demand restores any depth within seconds.",
    costPerDoc: "~0.06 KB",
...
function computeDepth(
  ...
  depth: StorageDepth;
  reason: string;
  signals: { label: string; value: number; weight: number; color: string }[];
} {
  // Signal weights
  ...
  const score =
    (accessSignal * 0.45 + trustSignal * 0.3 + relevanceSignal * 0.25) *
    ageFactor;
...
function buildTimeline(
  ...
  for (const day of checkpoints) {
    ...
    events.push({
      ...
      depth,
      signal: changed
        ? depthIndex(depth) < depthIndex(prev?.depth ?? "url_only")
          ? "demotion"
          : "promotion"
        : undefined,
    });
...
const DEPTH_ORDER: StorageDepth[] = [
  "url_only",
  "summary_only",
  "metadata",
  "full_text",
...
function depthIndex(d: StorageDepth) {
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-16">
      ...
      >
        <div className="flex items-center gap-2 font-mono text-xs text-primary mb-4 border border-primary/20 bg-primary/5 rounded px-3 py-1.5 w-fit">
          ...
          <span>
            memory.manager — intelligent promotion and demotion engine
          </span>
        </div>
        <h1 className="text-4xl font-bold mb-3">Memory Manager</h1>
        <p className="text-muted-foreground max-w-2xl leading-relaxed">
          Documents are promoted or demoted based on access frequency, source
          trust, and relevance to active learning goals. A document crawled once
          and never accessed becomes a URL-only record within days. The agent
          doesn't forget — it knows exactly where to look things up again.
        </p>
        {liveKnowledge && (
          <div className="mt-5 flex flex-wrap items-center gap-4 px-4 py-2.5 rounded-lg border border-primary/20 bg-primary/5 w-fit">
            ...
            {[
              {
                label: "Nodes indexed",
                value: liveKnowledge.nodeCount.toLocaleString(),
                color: "#22d3ee",
              },
              {
                label: "Graph edges",
                value: liveKnowledge.edgeCount.toLocaleString(),
                color: "#a78bfa",
              },
              {
                label: "Learning events",
                value: liveKnowledge.logCount.toLocaleString(),
                color: "#34d399",
              },
            ].map((s) => (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-mono text-sm text-primary mb-1">
                URLs are infinite free storage
              </p>
              <p className="font-mono text-xs text-muted-foreground leading-relaxed">
                A URL record costs 0.06 KB. Storing every URL the agent has ever
                encountered — all 5.4 billion indexed web pages — costs 324 GB
                at the network level. The agent retains knowledge of every URL's
                existence indefinitely. "Forgetting" is not deletion: it is
                returning a document to the internet, where it already lives,
          ...
          {[
            {
              ...
              threshold:
                "≥ 3 accesses within 7 days → metadata. ≥ 10 accesses → full text candidate.",
            },
            {
              ...
              threshold:
                "Trust < 0.4 → document cannot be promoted above summary_only regardless of access count.",
            },
            {
              ...
              threshold:
                "Relevance > 0.8 → hard promotion override: at least metadata depth, regardless of access count or age.",
            },
          ].map((s) => (
            <div
              key={s.label}
          ...
          <div className="md:col-span-2 bg-card border border-border rounded-lg p-5 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                max={90}
                step={1}
                value={daysSinceCrawl}
                onChange={(e) => setDaysSinceCrawl(Number(e.target.value))}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div
                    className="ml-auto font-mono text-xs px-2 py-1 rounded border"
                    style={{
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          ...
          <div className="bg-card border border-border rounded-lg p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              {(["never", "once_early", "frequent", "bursty"] as const).map(
                (p) => (
                  <button
                    key={p}
          ...
          <div className="bg-card border border-border rounded-lg p-4 space-y-4">
            ...
            <input
              ...
              step={0.01}
              value={timelineTrust}
              onChange={(e) => setTimelineTrust(Number(e.target.value))}
              className="w-full accent-primary"
            />
            <div className="flex justify-between font-mono text-[10px] text-muted-foreground">
              <span>0.0</span>
              <span className="text-primary font-bold">
                {timelineTrust.toFixed(2)}
              </span>
              <span>1.0</span>
            </div>
            <p className="font-mono text-xs text-muted-foreground">
            ...
            <input
              ...
              max={1}
              step={0.01}
              value={timelineRelevance}
              onChange={(e) => setTimelineRelevance(Number(e.target.value))}
              className="w-full accent-violet-500"
            />
            <div className="flex justify-between font-mono text-[10px] text-muted-foreground">
              <span>0%</span>
              <span className="text-violet-400 font-bold">
                {(timelineRelevance * 100).toFixed(0)}%
              </span>
              <span>100%</span>
            </div>
          </div>
            ...
            <div className="relative h-4 mt-1">
              {timeline.map((ev, i) => {
                ...
                return (
                  <span
                    key={i}
                    className="absolute font-mono text-[9px] text-muted-foreground/50 -translate-x-1/2"
                    style={{ left: `${leftPct}%` }}
                  >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="px-5 py-2 text-left font-mono text-[10px] text-muted-foreground uppercase tracking-wider w-16">
                    Day
                  </th>
                  <th className="px-3 py-2 text-left font-mono text-[10px] text-muted-foreground uppercase tracking-wider w-32">
                    Event
                  </th>
                  <th className="px-3 py-2 text-left font-mono text-[10px] text-muted-foreground uppercase tracking-wider w-32">
                    Depth
                  </th>
                  <th className="px-3 py-2 text-left font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
                    Outcome
                  </th>
                </tr>
              </thead>
      ...
      >
        <div className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-5">
          Demotion schedule — unaccessed documents
        </div>
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <table className="w-full">
            <thead>
            ...
            <tbody>
              {[
                {
                  age: "Day 0–1",
                  depth: "metadata" as StorageDepth,
                  dropped: "—",
                  note: "Grace period: full metadata retained while evaluation runs",
                },
                {
                  age: "Day 2–3",
                  depth: "summary_only" as StorageDepth,
                  dropped: "Full embedding, title/OG, provenance chain",
                },
                {
                  age: "Day 4–7",
                  depth: "summary_only" as StorageDepth,
                  dropped: "— (holding at summary, awaiting day-7 gate)",
                },
                {
                  age: "Day 7+",
                  depth: "url_only" as StorageDepth,
                  dropped:
                    "Summary, quantised embedding, trust record — URL retained forever",
                },
              ].map((row, i) => {
          ...
          <div className="border-t border-border px-5 py-3 bg-primary/5">
            <div className="flex items-start gap-2">
              ...
              <p className="font-mono text-[10px] text-muted-foreground">
                Any access resets the demotion clock. A single retrieval at day
                6 restores the document to full metadata depth and restarts the
                7-day evaluation window. Re-crawl on demand from URL-only state
                takes 2–8 seconds and restores any depth level.
              </p>
            </div>
          </div>
        </div>
