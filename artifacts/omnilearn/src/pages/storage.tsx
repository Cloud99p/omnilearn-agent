...
const TIERS: TierRow[] = [
  {
    ...
    maxAge: "≤ 30 days",
    condition:
      "Access frequency > 3 reads / 7-day window OR trust score ≥ 0.85",
    color: "#f97316",
  ...
  },
  {
    ...
    maxAge: "31–180 days",
    condition:
      "Access frequency 1–2 reads / 7-day window AND trust score 0.5–0.84",
    color: "#22d3ee",
...
const DEDUP_SIGNALS = [
  {
    signal: "Exact SHA-256 match",
    action: "Drop — identical document already indexed",
    reduction: "~40%",
  },
  {
    signal: "SimHash distance < 3 bits",
    action: "Merge embeddings, keep highest-trust copy",
    reduction: "~18%",
  },
  {
    signal: "Canonical URL redirect chain",
    action: "Collapse to canonical, drop all aliases",
    reduction: "~9%",
  },
  {
    signal: "Syndicated content fingerprint",
    action: "Store once, record all referring URLs as aliases",
    reduction: "~7%",
  },
  {
    signal: "Cross-agent federation dedup",
    action: "Skip ingest if any peer agent holds the record",
    reduction: "~12%",
  },
];
...
const SHARD_STRATEGIES = [
  {
    ...
    tradeoff:
      "Hotspot risk on high-volume domains. Mitigated by virtual nodes (128 vnodes per agent).",
  },
  {
    ...
    tradeoff:
      "Storage overhead: approximately 2.1× average for HIGH-trust corpus.",
  },
  {
    ...
    tradeoff:
      "Shard rebalancing required at month boundary. Offset with rolling 5-day overlap windows.",
  },
];
    ...
    <div className="space-y-1">
      <div className="flex justify-between items-center">
        ...
        <span
          ...
          style={{ color: overflow ? "#ef4444" : color }}
        >
          {formatBytes(value)}
          {overflow ? " — EXCEEDS AGENT CAPACITY" : ""}
        </span>
      </div>
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-16">
      ...
      >
        ...
        <p className="text-muted-foreground max-w-2xl leading-relaxed">
          The agent can't fulfil "the whole internet as data centre" if it
          chokes at a few million documents. Storage failure is a hard ceiling
          on growth. This page makes the math visible and shows how the
          ...
          <div className="mt-5 inline-flex items-center gap-3 px-4 py-2.5 rounded-lg border border-primary/20 bg-primary/5">
            ...
            <span className="font-mono text-xs text-muted-foreground">
              ...
              <span className="text-primary font-bold">
                {liveNodes.nodeCount.toLocaleString()} nodes
              </span>
              {" / "}
              <span className="text-foreground">
                {liveNodes.edgeCount.toLocaleString()} edges
              </span>
              {" — "}
              <span className="text-muted-foreground">
                {((liveNodes.nodeCount * 1.8) / 1024).toFixed(3)} MB at
                metadata-first scale
              </span>
            </span>
          </div>
        )}
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {[
            {
              label: "Indexed web pages (2024 est.)",
              value: "~5.4 billion",
              sub: "Common Crawl April 2024 snapshot",
              color: "#64748b",
            },
            {
              label: "Naive storage at 52 KB/page",
              value: "~280 TB",
              sub: "Per agent, before deduplication",
              color: "#ef4444",
            },
            {
              label: "Metadata-first at 1.8 KB/page",
              value: "~9.7 TB",
              sub: "Network total, federated across 10 agents",
              color: "#22d3ee",
            },
          ].map((c) => (
            <div
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-mono text-sm text-foreground mb-1">
                Why naive storage collapses
              </p>
              <p className="font-mono text-xs text-muted-foreground leading-relaxed">
                A naive crawler stores the full extracted text of every page. At
                52 KB average (compressed HTML, de-noised), one million pages
                costs 52 GB. One billion pages costs 52 TB — on a single agent.
                That's before replication, before index overhead, before
                embedding vectors. The ceiling is hit at a few million documents
                on any commodity hardware. Scaling the hardware doesn't solve
                the problem — it just moves the ceiling and compounds the cost.
              </p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-mono text-sm text-foreground mb-1">
                Why metadata-first survives it
              </p>
              <p className="font-mono text-xs text-muted-foreground leading-relaxed">
                A metadata record is a URL, a model-generated summary (3–5
                sentences), a 1536-dim embedding vector, a crawl timestamp, and
                a trust score. Total: ~1.8 KB. One billion pages costs 1.8 TB
                distributed across a federation of agents — feasible on standard
                cloud object storage. Copyrighted content is not absent from the
                agent's knowledge; it is represented without being owned.
              </p>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-card border border-border rounded-lg p-5 space-y-5">
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {(["naive", "metadata", "federated"] as StorageMode[]).map(
                  (m) => (
                    <button
                      key={m}
                      onClick={() => setMode(m)}
                      ...
                      style={
                        mode === m
                          ? {
                              borderColor: modeColor[m] + "80",
                              color: modeColor[m],
                              backgroundColor: modeColor[m] + "10",
                            }
                          : {}
                      }
                    >
          ...
          <div className="md:col-span-2 space-y-4">
            <div
              className={`bg-card border rounded-lg p-5 space-y-5 ${ceiling ? "border-red-500/40" : "border-border"}`}
            >
              ...
              <div className="border-t border-border pt-4 space-y-2">
                <p className="font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
                  All models at this document count
                </p>
                {(["naive", "metadata", "federated"] as StorageMode[]).map(
                  (m) => {
                    const { singleAgentGB: s } = calcStorage(
                      docs,
                      m,
                      agentCount,
                    );
                    ...
                    return (
                      <div key={m} className="flex items-center gap-3">
                        <div
                          ...
                          style={{ backgroundColor: modeColor[m] }}
                        />
                        <span className="font-mono text-xs text-muted-foreground w-36">
                          {modeLabel[m]}
                        </span>
                        ...
                        <span
                          className="font-mono text-xs w-28 text-right"
                          style={{ color: over ? "#ef4444" : modeColor[m] }}
                        >
                          {formatBytes(s)}
                          {over ? " ✗" : " ✓"}
                        </span>
              ...
              {ceiling && mode === "naive" && (
                <div className="bg-red-950/30 border border-red-500/30 rounded p-3">
                  <p className="font-mono text-xs text-red-300">
                    At {docCount}M documents, naive storage requires{" "}
                    {formatBytes(singleAgentGB)} per agent —{" "}
                    {(singleAgentGB / AGENT_CAPACITY_GB).toFixed(1)}× the
                    assumed 500 GB capacity. Switch to metadata-first to reduce
                    per-agent footprint by {((1 - 1.8 / 52) * 100).toFixed(0)}%.
                    Enable federation to distribute the remainder across{" "}
                    {agentCount} agents.
                  </p>
                </div>
              )}
      ...
      >
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            <p className="font-mono text-xs text-muted-foreground">
              26% of the raw corpus is eliminated before storage through five
              deduplication passes. Each pass runs on content fingerprints — no
              full text is materialised to perform dedup.
            </p>
          </div>
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="px-5 py-3 text-left font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
                  Signal
                </th>
                <th className="px-5 py-3 text-left font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
                  Action
                </th>
                <th className="px-5 py-3 text-right font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
                  Reduction
                </th>
              </tr>
            </thead>
            <tbody>
              {DEDUP_SIGNALS.map((row, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-primary text-right">
                    {row.reduction}
                  </td>
                </tr>
              ))}
            ...
            <tfoot>
              <tr className="border-t border-border bg-primary/5">
                <td
                  ...
                  colSpan={2}
                >
                  Combined effective reduction (with overlap)
                </td>
                <td className="px-5 py-3 font-mono text-xs font-bold text-primary text-right">
                  ~26%
                </td>
              </tr>
            </tfoot>
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="flex gap-8 font-mono text-xs text-muted-foreground">
            <div className="flex items-center gap-2">
              ...
              <span>
                Hot corpus: ~8% of indexed docs, ~31% of queries served
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-primary" />
              <span>
                Warm corpus: ~22% of indexed docs, ~54% of queries served
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-slate-500" />
              <span>
                Cold corpus: ~70% of indexed docs, ~15% of queries served
              </span>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="space-y-3">
          {SHARD_STRATEGIES.map((s, i) => (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="font-mono text-xs text-primary/50 w-6 shrink-0 mt-0.5">
                  0{i + 1}
                </div>
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
            ...
            <tbody>
              {[
                ...
                >
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-red-400 text-right">
                    {row.naive}
                  </td>
                  <td className="px-5 py-3 font-mono text-xs text-primary text-right">
                    {row.meta}
                  </td>
                  <td className="px-5 py-3 font-mono text-xs text-green-400 text-right">
                    {row.fed10}
                  </td>
          ...
          <div className="border-t border-border px-5 py-3">
            <p className="font-mono text-[10px] text-muted-foreground">
              Federated column assumes 10 agents, 500 GB/agent. Full web
              requires ~20 agents at metadata-first rates, or ~560 agents naive.
              Deduplication (26%) applied to all figures.
            </p>
          </div>
        </div>
      </motion.section>
