...
const SCHEMA = [
  {
    field: "data_sources[].type",
    type: "string",
    default: "crawler",
    desc: "Connector type: crawler | rss | api | db",
  },
  {
    field: "data_sources[].max_depth",
    type: "integer",
    default: "3",
    desc: "Max link-follow depth for crawlers",
  },
  {
    field: "model.name",
    type: "string",
    default: "Llama-3-8b",
    desc: "HuggingFace model ID or local path",
  },
  {
    field: "model.endpoint",
    type: "string",
    default: "local",
    desc: "'local' or remote inference URL",
  },
  {
    field: "model.context_window",
    type: "integer",
    default: "8192",
    desc: "Token context window for the model",
  },
  {
    field: "model.quantization",
    type: "string | null",
    default: "q4_k_m",
    desc: "GGUF quant level. null for full precision",
  },
  {
    field: "learning.interval_seconds",
    type: "integer",
    default: "3600",
    desc: "How often the learning pipeline triggers",
  },
  {
    field: "learning.batch_size",
    type: "integer",
    default: "512",
    desc: "Documents per learning batch",
  },
  {
    field: "learning.mode",
    type: "string",
    default: "active",
    desc: "passive | active | synthesis",
  },
  {
    field: "learning.finetune.enabled",
    type: "boolean",
    default: "true",
    desc: "Whether to run periodic LoRA fine-tuning",
  },
  {
    field: "learning.finetune.lora_rank",
    type: "integer",
    default: "16",
    desc: "LoRA adapter rank. Higher = more parameters",
  },
  {
    field: "personality.curiosity",
    type: "float [0,1]",
    default: "0.8",
    desc: "Controls breadth of topic exploration",
  },
  {
    field: "personality.skepticism",
    type: "float [0,1]",
    default: "0.6",
    desc: "Source credibility weighting bias",
  },
  {
    field: "personality.verbosity",
    type: "float [0,1]",
    default: "0.5",
    desc: "Response length preference",
  },
  {
    field: "personality.persona_version",
    type: "integer",
    default: "latest",
    desc: "Snapshot ID for inspection only. Core traits are permanent — snapshots cannot be fully restored.",
  },
  {
    field: "hardware.max_ram_gb",
    type: "integer",
    default: "64",
    desc: "System RAM cap for OmniLearn processes",
  },
  {
    field: "hardware.gpu_enabled",
    type: "boolean",
    default: "true",
    desc: "Enable CUDA/MPS acceleration",
  },
  {
    field: "hardware.crawler_workers",
    type: "integer",
    default: "8",
    desc: "Concurrent crawler threads",
  },
  {
    field: "ethics.robots_txt_respect",
    type: "boolean",
    default: "true",
    desc: "Enforce robots.txt. Disabling may violate law",
  },
  {
    field: "ethics.rate_limit_rps",
    type: "float",
    default: "2.0",
    desc: "Max requests/sec per domain",
  },
  {
    field: "ethics.mode",
    type: "string",
    default: "strict",
    desc: "strict | researcher. Neither allows illegal activity",
  },
  {
    field: "storage.hot_cache_gb",
    type: "integer",
    default: "8",
    desc: "In-memory cache for most-accessed embeddings",
  },
  {
    field: "storage.warm_index_gb",
    type: "integer",
    default: "100",
    desc: "SSD-backed vector index size",
  },
  {
    field: "storage.eviction_policy",
    type: "string",
    default: "lru",
    desc: "lru | priority | time_decay",
  },
];
        ...
        <div className="lg:w-48 shrink-0">
          ...
          <div className="space-y-1">
            {SECTIONS.map((s) => (
              ...
              >
                <ChevronRight
                  className={`w-3 h-3 ${activeSection === s.id ? "rotate-90" : ""} transition-transform`}
                />
                <span className={activeSection === s.id ? s.color : ""}>
                  {s.label}
                </span>
              </button>
            ))}
          ...
          <div className="mt-8 p-3 bg-card border border-border rounded-lg">
            ...
            <p className="text-xs text-muted-foreground">
              Changes detected via watchdog and applied without restart.
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm font-mono">
                  ...
                  <tbody>
                    {SCHEMA.filter(
                      (s) =>
                        !activeSection || s.field.startsWith(activeSection),
                    ).map((row, i) => (
                      <tr
                        key={row.field}
                        className={`border-b border-border/50 ${i % 2 === 0 ? "" : "bg-card/20"}`}
                      >
                        <td className="px-4 py-2.5 text-cyan-400 text-xs whitespace-nowrap">
                          {row.field}
                        </td>
                        <td className="px-4 py-2.5 text-violet-400 text-xs whitespace-nowrap">
                          {row.type}
                        </td>
                        <td className="px-4 py-2.5 text-emerald-400 text-xs whitespace-nowrap">
                          {row.default}
                        </td>
