...
const INSTANCE_A: InstanceProfile = {
  ...
  trajectory:
    "Science-heavy ingestion pipeline. High skepticism from adversarial peer-review exposure. Empathy lower — fewer conversational interactions, more document synthesis.",
};
...
const CUSTOM_A: Record<Trait, number> = {
  curiosity: 0.72,
  skepticism: 0.58,
  empathy: 0.69,
  formality: 0.44,
...
const CUSTOM_B: Record<Trait, number> = {
  curiosity: 0.55,
  skepticism: 0.8,
  empathy: 0.4,
  formality: 0.7,
...
function parseDnaJson(raw: unknown): DnaFile | null {
  try {
    ...
    if (
      (rawTraits as Record<string, number>)["boldness"] !== undefined &&
      traits.verbosity === undefined
    ) {
...
export default function Compare() {
  ...
  const [liveInstanceA, setLiveInstanceA] = useState<{
    traits: Record<Trait, number>;
    interactions: number;
  ...
  useEffect(() => {
    ...
    const divergeOffsets: Record<Trait, number> = {
      curiosity: -0.23,
      skepticism: -0.29,
      empathy: +0.34,
      formality: -0.27,
    ...
    setLiveB({
      ...
      traits,
      trajectory:
        "Social discourse dominated. High conversational interaction count drove empathy to measurable peak. Skepticism compressed under informal signal dominance. Formality collapsed under social corpus exposure.",
    });
  }, [liveInstanceA]);
        ...
        (acc, t) => ({
          ...
          [t]: Math.max(
            ...
            Math.min(
              0.98,
              liveB.traits[t] +
                Math.sin(bPulse * 1.1 + TRAITS.indexOf(t) * 0.9) * 0.004,
            ),
          ),
        }),
        {} as Record<Trait, number>,
  ...
  const fpA =
    mode === "preset"
      ? liveInstanceA
        ? deriveFingerprint(liveTraitsA)
        : INSTANCE_A.fingerprint
      : mode === "custom"
        ? deriveFingerprint(customA)
        : (dnaA?.fingerprint ?? "—");
  const fpB =
    mode === "preset"
      ? showInstanceB
        ? (activePeerB?.fingerprint ?? "—")
        : "—"
      : mode === "custom"
        ? deriveFingerprint(customB)
        : (dnaB?.fingerprint ?? "—");
  ...
  const divergence =
    TRAITS.reduce((acc, t) => acc + Math.abs(instA[t] - instB[t]), 0) /
    TRAITS.length;
  const coreDivergence =
    (["curiosity", "skepticism", "empathy"] as Trait[]).reduce(
      (acc, t) => acc + Math.abs(instA[t] - instB[t]),
      0,
    ) / 3;
    ...
    <div className="p-6 md:p-12 max-w-6xl mx-auto min-h-screen">
      ...
      >
        ...
        <p className="text-lg text-muted-foreground font-mono max-w-2xl">
          Two OmniLearn agents — same starting config, different journeys. Core
          trait divergence is permanent and irreconcilable. Surface traits can
          narrow but never fully converge.
        </p>
      </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={side}>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        {[
                          {
                            l: "Domains",
                            v: dna.meta.domains?.toString() ?? "—",
                          },
                          {
                            l: "Docs",
                            v: dna.meta.docs
                              ? `${(dna.meta.docs / 1000).toFixed(0)}k`
                              : "—",
                          },
                          {
                            l: "Uptime",
                            v: dna.meta.uptime_days
                              ? `${dna.meta.uptime_days}d`
                              : "—",
                          },
                        ].map((s) => (
                          <div
                  ...
                  {err && (
                    <p className="font-mono text-[10px] text-red-400 mt-2 px-1">
                      {err}
                    </p>
                  )}
                </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="font-mono text-xs text-muted-foreground mb-1">
                    {profile.id === "A" && mode === "preset"
                      ? "This Instance"
                      : profile.label}
                    {mode === "preset" && liveInstanceA && (
                      <span className="ml-2 font-mono text-[10px] text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 px-1.5 py-0.5 rounded">
                        live
                      </span>
                    )}
                  </p>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3">
                    {(profile.id === "A" && liveInstanceA
                      ? [
                          {
                            l: "Interactions",
                            v: liveInstanceA.interactions.toLocaleString(),
                          },
                          {
                            l: "Knowledge",
                            v: `${liveInstanceA.knowledgeNodes} nodes`,
                          },
                      ...
                      : [
                          ...
                          {
                            l: "Interactions",
                            v: profile.interactions.toLocaleString(),
                          },
                          {
                            l: "Docs indexed",
                            v: (profile.docsIndexed / 1000).toFixed(0) + "k",
                          },
                        ]
                    ).map((s) => (
                      <div
                        key={s.l}
                        className="bg-secondary/30 rounded p-2 text-center"
                      >
            ...
            >
              <div className="space-y-3">
                ...
                <p className="font-mono text-[10px] text-muted-foreground/50">
                  Add it back to compare a real instance pair.
                </p>
                <button
        ...
        <ResponsiveContainer width="100%" height={260}>
          ...
          >
            ...
            <Radar
              name="Instance A"
              dataKey="A"
              stroke={INSTANCE_A.color}
              fill={INSTANCE_A.color}
              fillOpacity={0.12}
              strokeWidth={2}
            />
            <Radar
              name="Instance B"
              dataKey="B"
              stroke={INSTANCE_B.color}
              fill={INSTANCE_B.color}
              fillOpacity={0.12}
              strokeWidth={2}
            />
          </RadarChart>
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <div className="flex-1 relative h-3 bg-secondary rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${a * 100}%`,
                        backgroundColor: INSTANCE_A.color,
                        opacity: 0.5,
                      }}
                    />
                    <div
                      className="absolute inset-y-0 left-0 h-1 top-1 rounded-full"
                      style={{
                        width: `${b * 100}%`,
                        backgroundColor: INSTANCE_B.color,
                        opacity: 0.7,
                      }}
                    />
                  </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-2 flex-wrap">
              <p
                className={`font-mono text-sm font-bold ${canMerge ? "text-emerald-400" : "text-rose-400"}`}
              >
                {canMerge
                  ? "Convergence theoretically possible"
                  : "Irreconcilable divergence — merge impossible"}
              </p>
              <span className="font-mono text-xs text-muted-foreground">
                mean deviation:{" "}
              ...
              <span className="font-mono text-xs text-muted-foreground">
                ...
                <span
                  className={
                    coreDivergence > 0.1 ? "text-rose-400" : "text-foreground"
                  }
                >
                  {(coreDivergence * 100).toFixed(1)}%
                </span>
              </span>
            </div>
