...
function CopyButton({ value, label }: { value: string; label?: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="flex items-center gap-1 text-muted-foreground hover:text-primary transition-colors font-mono text-xs"
    >
      {copied ? (
  ...
  return (
    ...
    >
      <div className="p-4">
          ...
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {node.isSelf && (
                  <span className="px-1.5 py-0.5 rounded font-mono text-[9px] uppercase tracking-wider bg-primary/10 border border-primary/20 text-primary">
                    This instance
                  </span>
                )}
                <span className="px-1.5 py-0.5 rounded font-mono text-[9px] uppercase tracking-wider bg-secondary/30 text-muted-foreground">
                  {node.region}
                </span>
              </div>
              <a
        ...
        {node.notes && (
          <p className="mt-2 font-mono text-xs text-muted-foreground/50 italic">
            {node.notes}
          </p>
        )}
      </div>
  ...
  return (
    <div className="rounded-xl border border-border/40 bg-card/30 p-5 space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="font-mono text-sm font-bold text-foreground">
            Create GitHub Deployment Repo
          </h3>
          <p className="font-mono text-xs text-muted-foreground mt-0.5">
            Generate a ready-to-deploy GitHub repository containing the ghost
            node server, Docker Compose config, setup scripts, and
            documentation.
          </p>
        </div>
      </div>
...
export default function GhostNetworkPage() {
  ...
  const pingAll = async () => {
    setPingingAll(true);
    try {
      const res = await fetch(`${BASE}/api/ghost/nodes/ping-all`, {
        method: "POST",
      });
  ...
  const generateInvite = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/ghost/worker/invite`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          label: inviteLabel.trim() || "Worker invite",
          maxUses: 100,
        }),
      });
      if (res.ok) {
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-muted-foreground">
              Distributed compute nodes for OmniLearn
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          {[
            ...
            },
            {
              ...
              value:
                status.totalTasksProcessed +
                browserWorkers.reduce((s, w) => s + w.tasksProcessed, 0),
              icon: Zap,
              color: "text-primary",
            },
            {
      ...
      <div className="mb-6 rounded-xl border border-border/40 bg-card/20 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-sm font-bold text-foreground">
              How Ghost Mode works — Setup guide
            </span>
          </div>
          {setupExpanded ? (
              ...
              <div className="px-5 pb-5 space-y-5 border-t border-border/30">
                <div className="mt-4 space-y-1">
                  <p className="font-mono text-xs text-muted-foreground leading-relaxed">
                    Ghost Mode turns every device you own into one connected web
                    of compute. Each ghost node can run anywhere — laptop,
                    desktop, home server, phone, cloud VM, or another Replit
                    deployment — and your tasks can move between them without
                    breaking continuity.
                  </p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-3">
                  {[
                    {
                      step: "1",
                      title: "Deploy a ghost node",
                      desc: "Run the ghost node server on any machine with Node.js or Docker.",
                    },
                    {
                      step: "2",
                      title: "Register it here",
                      desc: "Add the node's URL and shared secret. OmniLearn will ping it to verify.",
                    },
                    {
                      step: "3",
                      title: "Chat in Ghost Mode",
                      desc: "Switch to Ghost mode in Chat. Messages route to the best available node.",
                    },
                  ].map(({ step, title, desc }) => (
                    <div
                      key={step}
                ...
                <div>
                  <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-2">
                    Quick deploy — Node.js (no Docker)
                  </p>
                  <CodeBlock
                ...
                <div>
                  <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-2">
                    Register this instance as a ghost node
                  </p>
                  <p className="font-mono text-xs text-muted-foreground/70 mb-2">
                    You can register this OmniLearn instance itself as a ghost
                    node so other OmniLearn installs can route tasks to it. Set{" "}
                    <code className="bg-black/30 px-1 rounded text-primary/80">
                      GHOST_NODE_SECRET
                    </code>{" "}
                    in your environment secrets, then add the node below using
                    your own URL.
                  </p>
                  <CodeBlock
                    language="text"
      ...
      <div className="space-y-3 mb-6">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-border/40 p-10 text-center">
            ...
            <p className="font-mono text-sm text-muted-foreground/60 mb-1">
              No ghost nodes yet
            </p>
      ...
      <div className="mb-6 rounded-xl border border-violet-500/20 bg-card/20 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-sm font-bold text-foreground">
              Invite contributors — browser workers
            </span>
            ...
            >
              <div className="px-5 pb-5 border-t border-border/30 space-y-5">
                <p className="font-mono text-xs text-muted-foreground leading-relaxed mt-4">
                  Share an invite link with any device. When it opens, the
                  browser joins the ghost network and processes AI tasks using
                  its local synthesizer — no server required. That device
                  becomes part of the same connected web.
                </p>
                  ...
                  {[
                    {
                      ...
                      desc: "Generate a shareable token below. Anyone with the link can join.",
                    },
                    {
                      step: "2",
                      title: "Contributor opens it",
                      desc: "They enter a name. Their browser becomes a worker with local AI processing.",
                    },
                    {
                      step: "3",
                      title: "Tasks distribute automatically",
                      desc: "Ghost Mode routes tasks to online browser workers alongside server nodes.",
                    },
                  ].map(({ step, title, desc }) => (
                    <div
                      key={step}
      ...
      {browserWorkers.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={fetchData}
              className="font-mono text-xs text-muted-foreground/50 hover:text-muted-foreground transition-colors flex items-center gap-1"
            >
              ...
              >
                ...
                <div className="flex items-center gap-4 shrink-0">
                  {[
                    ...
                    {
                      label: "failed",
                      value: worker.tasksFailed,
                      color: "#f87171",
                    },
                    {
                      label: "avg",
                      value: worker.avgResponseMs
                        ? `${(worker.avgResponseMs / 1000).toFixed(1)}s`
                        : "—",
                      color: "#22d3ee",
                    },
                  ].map((s) => (
