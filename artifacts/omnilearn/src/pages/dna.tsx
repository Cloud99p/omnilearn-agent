...
function buildSnapshot(
  ...
  const traits = Object.fromEntries(
    TRAIT_META.map((t) => [t.label, +(char[t.key] / 100).toFixed(4)]),
  );
  const uptimeDays = Math.floor(
    (Date.now() - new Date(char.createdAt).getTime()) / 86_400_000,
  );
  return {
    instance: {
      id: fp,
    ...
    knowledge: {
      ...
      by_type: Object.fromEntries(
        knowledge.typeCounts.map((tc) => [tc.type, Number(tc.count)]),
      ),
    },
    substrate: {
      mode: "single_machine",
...
export default function DnaPage() {
  ...
  const uptimeDays = char
    ? Math.floor((Date.now() - new Date(char.createdAt).getTime()) / 86_400_000)
    : 0;
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-muted-foreground max-w-2xl leading-relaxed">
          A complete snapshot of this agent's live state — its character
          fingerprint, knowledge graph, trait vector, and substrate config.
          Pulled from the live database on every load. No two agents produce the
          same file.
        </p>
      </motion.div>
      ...
      >
        ...
        {[
          {
            label: "Created",
            value: char ? new Date(char.createdAt).toLocaleDateString() : "—",
          },
          { label: "Uptime", value: char ? `${uptimeDays}d` : "—" },
          {
            label: "Interactions",
            value: char ? char.totalInteractions.toLocaleString() : "—",
          },
          {
            label: "Knowledge",
            value: knowledge ? `${knowledge.nodeCount} nodes` : "—",
          },
        ].map((s) => (
          <div key={s.label}>
          ...
          {loading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="animate-pulse h-10 bg-secondary rounded"
                />
              ))}
            </div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3 mb-5">
                {[
                  {
                    label: "Nodes",
                    value: knowledge?.nodeCount ?? 0,
                    color: "#22d3ee",
                  },
                  {
                    label: "Edges",
                    value: knowledge?.edgeCount ?? 0,
                    color: "#a78bfa",
                  },
                  {
                    label: "Events",
                    value: knowledge?.logCount ?? 0,
                    color: "#34d399",
                  },
                ].map((s) => (
                  <div
              ...
              {knowledge?.typeCounts && knowledge.typeCounts.length > 0 && (
                <div className="space-y-3">
                  {knowledge.typeCounts.map((tc, i) => {
                    ...
                    const total = knowledge.typeCounts.reduce(
                      (a, t) => a + Number(t.count),
                      0,
                    );
                    const pct =
                      total > 0 ? (Number(tc.count) / total) * 100 : 0;
                    return (
                      <div key={tc.type}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-secondary/30">
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-xs text-muted-foreground">
              omnilearn-dna-{fingerprint}.json
            </span>
          </div>
          <div className="flex items-center gap-2">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            {
              title: "Migrate",
              desc: "Pass to `omnilearn restore --dna ./file.json` on a new machine. Character and knowledge transfer intact.",
            },
            {
              title: "Compare",
              desc: "Load two DNA files into the Compare page to see how two instances diverged from a common ancestor.",
            },
            {
              title: "Archive",
              desc: "Store as a point-in-time snapshot. Useful before major knowledge domain shifts or substrate changes.",
            },
          ].map((u) => (
            <div key={u.title}>
