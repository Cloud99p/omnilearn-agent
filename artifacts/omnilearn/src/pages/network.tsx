...
const CONCEPTS = [
  {
    ...
    detail: [
      "Your laptop (primary execution)",
      "Your desktop (secondary)",
      "Home server / NAS",
      "Mobile device (browser worker)",
      "Seamless resume on device wake",
      "No data leaves your device boundary without consent",
    ],
  },
  {
    id: "volunteer",
    ...
    detail: [
      "Tier 1: owner hardware — always first",
      "Tier 2: volunteer nodes — explicit opt-in required",
      "Tier 3: community infra — opt-out at any time",
      "All routed tasks visible to node operator",
      "Volunteer node can reject task categories",
      "Zero silent migration to third-party compute",
    ],
  },
  {
    id: "gossip",
    ...
    detail: [
      "Delta-compressed state diffs",
      "Merkle-tree knowledge hashes",
      "Content-addressed chunk storage",
      "Peer discovery via DHT",
      "Quorum validation before merge",
      "Conflict resolution by timestamp + trust score",
    ],
  },
  {
    id: "checkpoint",
    ...
    detail: [
      "AES-256 encrypted shards",
      "IPFS content addressing",
      "Arweave permanent storage layer",
      "Shamir secret sharing for the shard key",
      "Resumable from any node with key access",
      "State version vector for conflict detection",
    ],
  },
  {
    id: "stealth",
    ...
    detail: [
      "Per-node rate limiting (<2 req/s)",
      "robots.txt enforced locally",
      "Rotating user-agent with honest disclosure",
      "Trust score per domain, propagated via gossip",
      "Ethics governor on every ghost node",
  ...
  },
  {
    ...
    detail: [
      "Fingerprint = SHA-256(trait vector + trajectory hash)",
      "Shard key = Shamir(owner private key, threshold=3)",
      "Node proves identity via zero-knowledge proof",
      "No registration required — identity is emergent",
      "Two instances with same fingerprint are the same agent",
      "Fingerprints diverge after first independent crawl",
    ],
  },
];
...
export default function Network() {
  ...
  const runGossipProbe = useCallback(async () => {
    ...
    try {
      const d = await fetch(`${BASE}/api/ghost/gossip-probe`)
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/ghost/workers`)
      ...
      .then((d: unknown[]) =>
        setBrowserWorkers(
          Array.isArray(d)
            ? d.filter((w: unknown) => (w as { online: boolean }).online).length
            : 0,
        ),
      );
    runGossipProbe();
    ...
    <div className="p-6 md:p-12 max-w-6xl mx-auto min-h-screen">
      ...
      >
        ...
        <p className="text-lg text-muted-foreground font-mono max-w-2xl leading-relaxed">
          OmniLearn runs on your hardware first. When more compute is needed, it
          expands to explicitly consented volunteer nodes, then to community
          infrastructure — always with your knowledge, always reversible. No
          fixed address. No single point of failure or seizure.
        </p>
      </motion.div>
      ...
      >
        <div className="px-5 py-3 border-b border-border flex items-center gap-2">
          ...
          <span className="font-mono text-sm text-muted-foreground">
            execution_fabric.live — registered nodes:{" "}
            {ghostStatus ? ghostStatus.total : NODE_POSITIONS.length}
            {ghostStatus && ghostStatus.online > 0 && (
              <span className="ml-2 text-emerald-400">
                {ghostStatus.online} online
              </span>
            )}
          </span>
          <span className="ml-auto font-mono text-xs text-muted-foreground">
            {ghostStatus
              ? `${ghostStatus.totalTasksProcessed} tasks processed`
          ...
          return (
            <div className={`rounded-xl border p-5 ${meta.bg} ${meta.border}`}>
              ...
              <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                ...
                >
                  <RefreshCw
                    className={`w-3 h-3 ${probeLoading ? "animate-spin" : ""}`}
                  />
              ...
              {probe && probe.workers.length > 0 ? (
                <div className="space-y-2">
                  ...
                  {probe.workers.map((w, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 bg-background/40 rounded-lg px-3 py-2"
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${w.alive ? "bg-emerald-400 animate-pulse" : "bg-gray-600"}`}
                      />
                      ...
                      <span className="font-mono text-[10px] text-muted-foreground">
                        {w.tasksProcessed} tasks
                      </span>
                      <span
                        className={`font-mono text-[10px] ${w.alive ? "text-emerald-400" : "text-gray-500"}`}
                      >
                ...
                probe && (
                  <div className="bg-background/40 rounded-lg px-4 py-3 font-mono text-xs text-muted-foreground text-center">
                    No peer nodes registered — open the Ghost Network page and
                    run a worker to add one
                  </div>
                )
              )}
              ...
              {probe && state !== "active" && (
                <p className="font-mono text-[10px] text-muted-foreground mt-3 pt-3 border-t border-border/40">
                  The animated sync lines above are architectural illustrations.
                  Gossip protocol activates only when ≥{probe.minForGossip}{" "}
                  peers are online simultaneously.
                  {probe.probeTime &&
                    ` Last probe: ${new Date(probe.probeTime).toLocaleTimeString()}.`}
                </p>
              )}
              {probe && state === "active" && (
                <p className="font-mono text-[10px] text-emerald-400 mt-3 pt-3 border-t border-emerald-400/20">
                  ✓ Gossip protocol is live. Knowledge deltas can now propagate
                  between peers.
                  {probe.probeTime &&
                    ` Last probe: ${new Date(probe.probeTime).toLocaleTimeString()}.`}
                </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {CONCEPTS.map((c, i) => (
          ...
          >
            ...
            <div
              className="mt-3 flex items-center gap-1 font-mono text-[10px]"
              style={{ color: c.color }}
            >
        ...
        {activeConcept && (
          ...
          >
            <div
              ...
              style={{ borderColor: activeConcept.color + "40" }}
            >
              <div className="flex items-center gap-3 mb-4">
                <div
                  className="p-2.5 rounded-lg"
                  style={{ backgroundColor: activeConcept.color + "15" }}
                >
                  <activeConcept.icon
                    className="w-5 h-5"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {activeConcept.detail.map((d) => (
                  <div
                    key={d}
                    className="flex items-center gap-2 bg-secondary/40 rounded px-3 py-2"
                  >
                    <div
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed max-w-3xl">
              Traditional AI agents are bound to a machine — constrained by RAM,
              GPU, and storage. OmniLearn inverts this. The constraint becomes
              the distribution strategy. Limited local compute means the agent
              must spread. Spreading means it becomes fault-tolerant.
              Fault-tolerance means no single entity can shut it down. It exists
              wherever the internet exists — and the internet is everywhere.
            </p>
          </div>
        </div>
      </motion.div>
...
const PHASES = [
  {
    ...
    capabilities: [
      "Crawl & index content",
      "Sync knowledge deltas via gossip",
      "Propose improvements to the feed",
      "Accumulate domain & accuracy history",
    ],
    blocked: [
      "Cast votes on any proposal",
      "Have contributions weighted",
      "Influence quorum outcomes",
    ],
  ...
  },
  {
    ...
    badgeColor: "#facc15",
    capabilities: [
      "Cast votes (partial weight)",
      "Contributions counted toward quorum",
      "Trust score begins accumulating",
      "Relay routes diversifying",
    ],
  ...
  },
  {
    ...
    badgeColor: "#34d399",
    capabilities: [
      "Full voting weight per formula",
      "Weighted contributions to quorum",
      "Can co-sign relay introductions",
      "Trust score used as jury weight",
    ],
    blocked: [],
...
const WEIGHT_FORMULA_FACTORS = [
  {
    ...
    whyHard:
      "Takes weeks of real crawling across genuinely different domains. Mirrors of the same content don't count — deduplication runs on content hash.",
  },
  {
    id: "accuracy",
    ...
    whyHard:
      "Requires months of participation, each contribution independently evaluated by peers who have no reason to be biased. A bad actor submitting false data gets a low accuracy score permanently.",
  },
  {
    id: "topology",
    ...
    whyHard:
      "Different ASNs, routing prefixes, geographic regions. A thousand VMs in the same datacenter all look the same topologically — they contribute one topology unit, not a thousand.",
  },
];

function calcNodeWeight(
  domainCount: number,
  accuracyPct: number,
  topoPaths: number,
  ageDays: number,
  ...
  const domain = Math.min(
    1.0,
    Math.pow(domainCount / 50, 0.7) * Math.min(1.0, domainCount / 10),
  );
...
function ProofOfExistence() {
  ...
  const phase =
    ageDays < 30 ? "observer" : ageDays < 90 ? "probationary" : "member";
  const phaseColor =
    phase === "observer"
      ? "#f87171"
      : phase === "probationary"
    ...
    >
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground max-w-2xl leading-relaxed">
          New nodes join the network with zero voting weight. Influence cannot
          be bought, injected, or bootstrapped. It accumulates from real work
          across diverse domains, validated by independent peers over months —
          not milliseconds. Spinning up 10,000 nodes overnight gives you 10,000
          × 0 = 0 influence.
        </p>
      </div>
        ...
        >
          <div>
            <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
              <CheckCircle className="w-3.5 h-3.5 text-emerald-400" /> Allowed
              during this phase
            </p>
            <div className="space-y-2">
          ...
          <div>
            {activePhaseData.blocked.length > 0 && (
              <>
                <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                  <Vote className="w-3.5 h-3.5 text-red-400" /> Not yet
                  available
                </p>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-border">
          <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-1">
            Voting weight formula — interactive
          </p>
          <p className="font-mono text-[10px] text-muted-foreground">
            Adjust the sliders to see how each factor affects this node's
            influence.
          </p>
        </div>
        ...
        {WEIGHT_FORMULA_FACTORS.map((f) => (
          ...
          >
            ...
            <div className="bg-secondary/40 rounded px-2 py-1.5">
              <p className="font-mono text-[9px]" style={{ color: f.color }}>
                {f.formula}
              </p>
            </div>
            <p className="font-mono text-[10px] text-muted-foreground leading-relaxed mt-3 border-t border-border/50 pt-2">
              <span className="text-foreground">Why it's hard to fake:</span>{" "}
              {f.whyHard}
            </p>
          </div>
        ))}
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        >
          ...
          <span className="font-mono text-sm font-bold">
            Sybil resistance — why mass-spawning fails
          </span>
          <span className="ml-auto font-mono text-xs text-muted-foreground">
            {showSybil ? "collapse" : "expand"}
          </span>
        </button>
        <AnimatePresence>
          {showSybil && (
              ...
              <div className="px-6 pb-6 border-t border-border grid md:grid-cols-2 gap-6 pt-5">
                <div>
                  <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-4">
                    If an attacker spawns 10,000 nodes at once
                  </p>
                  <div className="space-y-3">
                    {[
                      {
                        label: "Each node's age",
                        value: "0 days",
                        result: "Voting weight = ×0",
                        bad: true,
                      },
                      {
                        label: "Domain diversity",
                        value: "All new — no history",
                        result: "domain_score = 0.0",
                        bad: true,
                      },
                      {
                        label: "Accuracy score",
                        value: "< 30 contributions",
                        result: "Not activated yet",
                        bad: true,
                      },
                      {
                        label: "Topology",
                        value: "Same datacenter",
                        result: "1 topology unit total",
                        bad: true,
                      },
                      {
                ...
                <div>
                  <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-4">
                    To achieve meaningful influence
                  </p>
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={i} className="flex items-start gap-2">
                        <span className="font-mono text-[10px] text-violet-400 w-4 shrink-0">
                          {i + 1}.
                        </span>
                  ...
                  <div className="mt-4 rounded-lg bg-violet-500/10 border border-violet-500/20 px-4 py-3">
                    <p className="font-mono text-[10px] text-violet-300 leading-relaxed">
                      Cost of a viable Sybil attack = months of real crawl time
                      × 10,000 nodes × real compute × ASN diversity. At that
                      investment level, the attacker is genuinely contributing
                      to the network's knowledge base — which is the point.
                    </p>
                  </div>
                </div>
              </div>
...
function makeContrib(): ContribEvent {
  const t =
    CONTRIBUTION_TYPES[Math.floor(Math.random() * CONTRIBUTION_TYPES.length)];
  const now = new Date();
  const ts = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}:${String(now.getSeconds()).padStart(2, "0")}`;
  const status = (
    ["proposed", "voting", "voting", "ratified"] as ContribEvent["status"][]
...
const INIT_CONTRIBS: ContribEvent[] = Array.from(
  { length: 6 },
  makeContrib,
).reverse();
...
function CollectiveEvolution({
  ...
  const [activeTab, setActiveTab] = useState<"growth" | "feed" | "protocol">(
    "growth",
  );
  ...
  return (
    ...
    >
      ...
      <div className="mb-8">
        ...
        <h2 className="text-3xl font-bold tracking-tight mb-3">
          The Network Grows With Every Instance
        </h2>
        <p className="text-muted-foreground max-w-2xl leading-relaxed">
          OmniLearn agents are not isolated. Each instance can optionally
          contribute anonymised learning deltas back to the collective. When
          enough independent agents agree on an improvement, it is ratified and
          propagated to every node via gossip. The more instances exist, the
          smarter the whole network becomes — and the faster it grows.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          ...
          {
            label: "Improvements ratified",
            value: improvementsRatified.toLocaleString(),
            color: "#a78bfa",
            icon: CheckCircle,
          },
          {
            label: "Contributions this epoch",
            value: contributionsThisEpoch.toLocaleString(),
            color: "#fb923c",
            icon: Upload,
          },
        ].map((s) => (
                  ...
                  >
                    ...
                    <Tooltip
                      contentStyle={{
                        ...
                        border: "1px solid hsl(214.3 31.8% 16%)",
                        borderRadius: 8,
                        fontFamily: "monospace",
                        fontSize: 11,
                      }}
            ...
            <div className="space-y-3">
              ...
              {liveMilestones.length === 0 ? (
                <div className="bg-card border border-border rounded-lg px-4 py-4 text-center font-mono text-xs text-muted-foreground">
                  No learning events yet — interact with the agent to generate
                  entries
                </div>
              ) : (
                liveMilestones.map((m) => (
                  <div
                    key={m.index}
                    className="flex items-start gap-4 bg-card border border-border rounded-lg px-4 py-3"
                  >
                    <span className="font-mono text-[10px] text-muted-foreground w-14 shrink-0 pt-0.5">
                      {m.age}
                    </span>
                    <div className="flex-1 font-mono text-sm text-foreground leading-snug">
                      {m.label}
                    </div>
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              ...
              <AnimatePresence initial={false}>
                {contribs.map((c) => (
                  ...
                  >
                    <span className="font-mono text-[10px] text-muted-foreground/60 w-16 shrink-0">
                      {c.ts}
                    </span>
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: c.type.color + "10",
                      }}
                    >
                      {c.type.label}
                    </div>
              ...
              {[
                {
                  ...
                  bullets: [
                    "Zero raw data leaves the instance",
                    "Contribution flag in omni_config.yaml",
                    "Can be disabled at any time",
                    "Gradient-only submissions",
                  ],
                },
                {
                  step: "02",
                  ...
                  bullets: [
                    "Merkle diff against collective baseline",
                    "Compressed to <4 KB per submission",
                    "Cryptographically signed by instance fingerprint",
                    "Submitted to nearest relay node",
                  ],
                },
                {
                  step: "03",
                  ...
                  bullets: [
                    "Vote weight = f(age, accuracy, diversity)",
                    "Threshold = sqrt(active_nodes)",
                    "Voting window = 48 hours",
                    "Abstain by default — no vote ≠ rejection",
                  ],
                },
                {
                  step: "04",
                  color: "#fb923c",
                  title: "Ratification & propagation",
                  ...
                  bullets: [
                    "Improvement hash appended to ledger",
                    "Gossiped with TTL=∞",
                    "Nodes verify signature before applying",
                    "Rollback possible if >30% nodes reject",
                  ],
                },
              ].map((s) => (
                <div
