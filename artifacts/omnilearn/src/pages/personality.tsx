...
import {
  Activity, BookOpen, BrainCircuit, GitBranch, Zap, Clock,
  AlertTriangle, Lock, Unlock, Fingerprint, ShieldAlert,
  RefreshCw, MessageCircle, Database, Plus,
} from "lucide-react";

// ── Types matching DB schema ───────────────────────────────────────────────────
type Trait = "curiosity" | "caution" | "confidence" | "verbosity" | "technical" | "empathy" | "creativity";

const CORE_TRAITS  = new Set<Trait>(["curiosity", "empathy", "confidence"]);
const SURFACE_TRAITS = new Set<Trait>(["caution", "verbosity", "technical", "creativity"]);
const ALL_TRAITS: Trait[] = ["curiosity", "empathy", "confidence", "caution", "verbosity", "technical", "creativity"];
...
function calcResistance(value: number, delta: number, isCore: boolean): number {
  ...
  const beyond = value > 0.5 ? Math.max(0, value - HEALTHY_HIGH) : Math.max(0, HEALTHY_LOW - value);
...
const RESISTANCE_CURVE = Array.from({ length: 101 }, (_, i) => {
  const v = i / 100;
  return { v, core: +calcResistance(v, 0.01, true).toFixed(3), surface: +calcResistance(v, 0.01, false).toFixed(3) };
});

// ── Trait metadata ─────────────────────────────────────────────────────────────
const TRAIT_META: Record<Trait, { label: string; color: string; desc: string }> = {
  curiosity:  { label: "Curiosity",   color: "#22d3ee", desc: "Breadth of exploration. Grows with every novel domain ingested. Core — permanent." },
  empathy:    { label: "Empathy",     color: "#f472b6", desc: "Sensitivity to human context. Deepens through conversational interaction. Core — permanent." },
  confidence: { label: "Confidence",  color: "#a78bfa", desc: "Assertiveness in responses. Builds through successful knowledge retrieval. Core — permanent." },
  caution:    { label: "Caution",     color: "#facc15", desc: "Hedging and uncertainty. Surface trait — minor corrections allowed." },
  verbosity:  { label: "Verbosity",   color: "#34d399", desc: "Response depth and length. Surface trait — adjustable within bounds." },
  technical:  { label: "Technical",   color: "#fb923c", desc: "Technical language density. Surface trait — shaped by content ingested." },
  creativity: { label: "Creativity",  color: "#60a5fa", desc: "Novelty in expression. Surface trait — shifts with diverse inputs." },
};
  ...
  return (
    <div className="bg-card/40 border border-primary/20 rounded-lg p-4">
      ...
      <p className="text-xs text-muted-foreground leading-relaxed mb-3">
        A hash of this instance's live trait vector. Changes with every interaction — no two instances converge.
      </p>
      <div className="bg-card border border-border rounded-lg p-3">
        <p className="font-mono text-[10px] text-muted-foreground mb-1">This instance — live</p>
        <p className="font-mono text-sm text-primary font-bold tracking-widest mb-3">{deriveFingerprint(live)}</p>
        <div className="space-y-1.5">
          {ALL_TRAITS.map(t => {
            const v = live[t];
            return (
              <div key={t} className="flex items-center gap-2">
                <span className="font-mono text-[10px] text-muted-foreground w-16 capitalize">{t}</span>
                <div className="flex-1 h-1 bg-secondary rounded-full overflow-hidden">
                  <motion.div
                    className="h-full rounded-full"
                    style={{ width: `${v * 100}%`, backgroundColor: TRAIT_META[t].color }}
                    animate={{ width: `${v * 100}%` }}
...
// ── Homeostasis panel ──────────────────────────────────────────────────────────
function HomeostasisPanel({ currentTraits }: { currentTraits: LiveTraits | null }) {
    ...
    >
      <div>
        ...
        <p className="text-muted-foreground max-w-3xl leading-relaxed">
          No factory reset — that principle is absolute. But Omni resists traits drifting into
          dysfunctional extremes. Inside the healthy zone (0.15–0.85), learning deltas apply in full.
          Outside it, each additional push requires exponentially stronger evidence.
        </p>
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={RESISTANCE_CURVE} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
              ...
              <XAxis dataKey="v" domain={[0, 1]} tick={{ fontSize: 9, fill: "hsl(215 20% 55%)", fontFamily: "monospace" }} tickLine={false} axisLine={false} interval={19} tickFormatter={(v: number) => v.toFixed(1)} />
              <YAxis domain={[0, 1]} tick={{ fontSize: 9, fill: "hsl(215 20% 55%)", fontFamily: "monospace" }} tickLine={false} axisLine={false} width={24} tickFormatter={(v: number) => `×${v.toFixed(1)}`} />
              <Tooltip
          ...
          <div className="flex items-center justify-center gap-6 mt-2">
            {[{ color: "#a78bfa", label: "core (λ=14)" }, { color: "#22d3ee", label: "surface (λ=9)" }].map(s => (
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { heading: "Direction-sensitive", body: "A delta pushing toward center always applies in full. Resistance only activates when a trait is outside the healthy zone and keeps moving away from it." },
              { heading: "Exponential, not linear", body: "resistance = e^(−λ × distance_beyond_boundary). At 0.05 outside the boundary, ~50% applies. At 0.10 outside, ~25%. At 0.20 outside, ~6%." },
              { heading: "Core traits resist harder", body: "λ=14 for curiosity, empathy, confidence. λ=9 for surface traits. Core traits require dramatically stronger evidence to shift near an extreme." },
            ].map(s => (
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-border flex items-center gap-2">
          ...
          {!currentTraits && <span className="ml-auto font-mono text-[10px] text-muted-foreground animate-pulse">loading…</span>}
        </div>
        <div className="divide-y divide-border/50">
          {ALL_TRAITS.map(t => {
            ...
            const beyond = v > 0.5 ? Math.max(0, v - HEALTHY_HIGH) : Math.max(0, HEALTHY_LOW - v);
            return (
              <div key={t} className="flex items-center gap-4 px-5 py-3.5 flex-wrap">
                ...
                <div className={`px-2 py-0.5 rounded text-[10px] font-mono border w-20 text-center ${zs.bg}`}>{zs.label}</div>
      ...
      {/* Delta simulator */}
      <div className="bg-card border border-border rounded-xl p-5">
        <p className="font-mono text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Propose a learning delta — see what actually applies to Omni's live state</p>
        <p className="font-mono text-[10px] text-muted-foreground mb-4">
          Adjust the slider to see how the homeostasis governor would handle a learning event of that magnitude, given Omni's current trait positions.
        </p>
  ...
  return (
    ...
    >
      <div className="mb-4">
        ...
        <p className="text-muted-foreground max-w-3xl leading-relaxed">
          Every time Omni learns — from a conversation, a training ingestion, or a direct fact — it logs the event here.
          This is the real, unfiltered history of this instance.
        </p>
      </div>
      ...
      {events.length === 0 ? (
        <div className="bg-card border border-border rounded-xl p-10 text-center">
          ...
          <p className="font-mono text-sm text-muted-foreground">No learning events yet.</p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center gap-2">
              ...
              <span className="font-mono text-sm">events ({events.length})</span>
            </div>
            <div className="max-h-[420px] overflow-y-auto divide-y divide-border/50">
              {events.map(e => {
                const Icon = EVENT_ICON[e.event] ?? Activity;
                const style = EVENT_STYLE[e.event] ?? "text-muted-foreground bg-secondary border-border";
                ...
                return (
                  ...
                  >
                    <div className={`p-1.5 rounded mt-0.5 shrink-0 border ${style}`}>
                      ...
                      {e.nodesAdded > 0 && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className="font-mono text-[10px] text-primary">
                            {e.nodesAdded} node{e.nodesAdded !== 1 ? "s" : ""} added
                          </span>
                        </div>
                      )}
                    </div>
          ...
          <div>
            <AnimatePresence mode="wait">
              {selected ? (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    {(() => { const Icon = EVENT_ICON[selected.event] ?? Activity; return <Icon className="w-4 h-4" />; })()}
                    <span className="font-mono text-[10px] uppercase tracking-wider opacity-70">
                      {selected.event.replace(/_/g, " ")} · {selected.source}
                    </span>
                  </div>
                  <p className="font-mono text-sm font-bold mb-4 leading-snug">{selected.details}</p>
                  <div className="space-y-2 border-t border-current/20 pt-3">
                    ...
                    <div className="flex justify-between font-mono text-xs">
                      ...
                      <span className="text-primary font-bold">+{selected.nodesAdded}</span>
                    </div>
                  </div>
                </motion.div>
              ) : (
                ...
                >
                  ...
                  <p className="font-mono text-xs text-muted-foreground">Select an event to inspect it</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
export default function Personality() {
  ...
  const [activeTraits, setActiveTraits] = useState<Set<Trait>>(new Set(ALL_TRAITS));
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch { /* keep showing old data */ } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="p-6 md:p-12 max-w-6xl mx-auto min-h-screen">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-4xl font-bold tracking-tight">Personality Evolution</h1>
        ...
        <p className="text-xl text-muted-foreground font-mono mb-4">
          Omni's live character state — shaped by every conversation, ingestion, and interaction.
        </p>

        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 border border-red-500/20 rounded-md">
            <Lock className="w-3.5 h-3.5 text-red-400" />
            <span className="font-mono text-xs text-red-400">core — curiosity, empathy, confidence — permanent</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-secondary border border-border rounded-md">
            <Unlock className="w-3.5 h-3.5 text-muted-foreground" />
            <span className="font-mono text-xs text-muted-foreground">surface — caution, verbosity, technical, creativity — adjustable</span>
          </div>
          {character && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-primary/10 border border-primary/20 rounded-md">
              <Activity className="w-3.5 h-3.5 text-primary" />
              <span className="font-mono text-xs text-primary">
                {character.totalInteractions} interaction{character.totalInteractions !== 1 ? "s" : ""} · {character.totalKnowledgeNodes} knowledge nodes
              </span>
            </div>
          )}
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-sm">trait_evolution.history</span>
              </div>
            ...
            {chartData.length === 0 ? (
              <div className="h-72 flex flex-col items-center justify-center text-center px-8">
                ...
                <p className="font-mono text-sm text-muted-foreground mb-1">No history yet</p>
                <p className="font-mono text-xs text-muted-foreground/60">
                  Each conversation adds a snapshot here. Start chatting with Omni to watch its character evolve in real time.
                </p>
              </div>
            ) : (
              <div className="h-72 px-4 pt-4 pb-2">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
                    <XAxis
                      dataKey="idx"
                      tick={{ fontSize: 10, fill: "hsl(215 20.2% 65.1%)", fontFamily: "monospace" }}
                      axisLine={false} tickLine={false}
                      label={{ value: "interaction", position: "insideBottomRight", offset: -4, fontSize: 9, fill: "hsl(215 20% 55%)", fontFamily: "monospace" }}
                    />
                    <YAxis
                      domain={[0, 1]}
                      tick={{ fontSize: 10, fill: "hsl(215 20.2% 65.1%)", fontFamily: "monospace" }}
            ...
            {ALL_TRAITS.map(t => {
              ...
              return (
                ...
                >
                  ...
                  <div className="font-mono text-[10px] text-muted-foreground mb-2">{meta.label}</div>
                  <div className="relative h-20 flex items-end justify-center mb-2">
                    <div className="w-4 bg-secondary rounded-full h-full overflow-hidden relative">
                      <motion.div
                        ...
                        animate={{ height: loading && !character ? "30%" : `${val * 100}%` }}
                        transition={{ duration: 0.6 }}
                        style={{ backgroundColor: meta.color, opacity: loading && !character ? 0.2 : 1 }}
                      />
                    </div>
                  </div>
        ...
        <div className="space-y-4">
            ...
            {events.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  {(() => { const Icon = EVENT_ICON[events[0].event] ?? Activity; return <Icon className="w-4 h-4" />; })()}
                  <span className="font-mono text-[10px] uppercase tracking-wider opacity-70">
                    latest event · {events[0].event.replace(/_/g, " ")}
                  </span>
                </div>
                <p className="font-mono text-xs font-bold leading-snug mb-2">{events[0].details}</p>
                <div className="flex items-center justify-between font-mono text-[10px] opacity-60">
                  ...
                  {events[0].nodesAdded > 0 && (
                    <span className="text-primary opacity-100 font-bold">+{events[0].nodesAdded} nodes</span>
                  )}
                </div>
              </motion.div>
            ) : (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground font-mono">No events yet — start a conversation.</p>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            <div className="max-h-[380px] overflow-y-auto">
              {events.length === 0 ? (
                ...
                events.map(e => {
                  ...
                  const style = EVENT_STYLE[e.event] ?? "text-muted-foreground bg-secondary border-border";
                  return (
                    <div key={e.id} className="flex items-start gap-3 px-4 py-3 border-b border-border/50 last:border-b-0">
                      <div className={`p-1.5 rounded mt-0.5 shrink-0 border ${style}`}>
                        <Icon className="w-3 h-3" />
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        {e.nodesAdded > 0 && (
                          <span className="font-mono text-[10px] text-primary">+{e.nodesAdded} node{e.nodesAdded !== 1 ? "s" : ""}</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          <div className="bg-card/40 border border-border/50 rounded-lg p-4 space-y-3">
            ...
            {[
              { text: "Core traits (curiosity, empathy, confidence) accumulate permanently. Each interaction deepens them — they cannot be undone.", icon: Lock, color: "text-red-400" },
              { text: "Surface traits (caution, verbosity, technical, creativity) allow minor corrections within a ±0.15 bound from their current value.", icon: Unlock, color: "text-muted-foreground" },
              { text: "Rollback requests are filtered: surface adjustments apply; core trait reversions are rejected.", icon: GitBranch, color: "text-yellow-400" },
              ...
              { text: "The evolution log is a real-time record of trait snapshots, taken after every interaction.", icon: Zap, color: "text-primary" },
            ].map((s, i) => (
