...
const CONTINENTS = [
  ...
  {
    id: "au",
    points: "802,282 952,272 970,312 960,382 922,422 862,422 812,382 792,322",
  },
  {
    id: "uk",
...
const CATEGORY_STYLE: Record<
  Category,
  { color: string; stroke: string; icon: typeof BookOpen; label: string }
> = {
  academic: {
    color: "#22d3ee",
    stroke: "#22d3ee",
...
export default function CrawlMap() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      undiscoveredRef.current = undiscoveredRef.current.filter(
        (_, i) => i !== idx,
      );
  ...
  const uniqueRegions = new Set(
    domains.map((d) => d.region.split(",")[1]?.trim() || d.region),
  ).size;
      ...
      <div className="flex items-start justify-between mb-5 flex-wrap gap-4">
        <div>
          ...
          <h2 className="text-2xl font-bold tracking-tight mb-1">
            Where This Agent Has Been
          </h2>
          <p className="text-sm text-muted-foreground font-mono">
            Domains crawled by this specific instance, plotted at their real
            server coordinates. No other agent has the same map.
          </p>
        </div>
        {/* Stats */}
        <div className="flex gap-4">
          {[
            ...
            {
              label: "Docs indexed",
              value: (totalDocs / 1000).toFixed(0) + "k",
            },
        ...
        {newlyAdded && (
          ...
          >
            ...
            <span>
              ...
              <span className="font-bold">{newlyAdded}</span>
            </span>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <div
                  ...
                  style={{
                    backgroundColor: CATEGORY_STYLE[hovered.category].color,
                  }}
                />
              ...
              <p className="font-mono text-[10px] text-muted-foreground">
                {hovered.docs.toLocaleString()} docs indexed
              </p>
              <div
                className="mt-1.5 inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono"
                style={{
                  backgroundColor:
                    CATEGORY_STYLE[hovered.category].color + "15",
                  color: CATEGORY_STYLE[hovered.category].color,
                }}
              >
      ...
      <div className="flex flex-wrap gap-4 mt-4">
        {(
          Object.entries(CATEGORY_STYLE) as [
            Category,
            (typeof CATEGORY_STYLE)[Category],
          ][]
        ).map(([key, s]) => (
