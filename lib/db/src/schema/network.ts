...
export const networkAgents = pgTable("network_agents", {
  ...
  phaseStartedAt: timestamp("phase_started_at", { withTimezone: true }).defaultNow(),
...
export const networkVotes = pgTable("network_votes", {
  id: serial("id").primaryKey(),
  neuronId: integer("neuron_id").notNull().references(() => networkNeurons.id, { onDelete: "cascade" }),
  agentName: text("agent_name").notNull().references(() => networkAgents.name, { onDelete: "cascade" }),
...
export const agentRelayPaths = pgTable("agent_relay_paths", {
  id: serial("id").primaryKey(),
  agentName: text("agent_name").notNull().references(() => networkAgents.name, { onDelete: "cascade" }),
